 ...
 methods:{
    goBack(){
       if(sysEnv!==1){
        ...
        let tabObj = {};
        tabObj.desc = '评审库'
        let goPage = "flowManage/index.html#/review";
        tabObj.r_func = "{menuTarget:'IFRAME',tabKey:'review',href_link:'" + goPage + "'}";
        tabObj.reload = true;
        tabObj.clearIframe = true;
        EcoUtil.getSysvm().doTab(tabObj);
        let that=this
        setTimeout(() => {
            window.parent.window.sysvm.removeTab('reviewDetail' + that.id);
        }, 100);
       }
    }
 }
