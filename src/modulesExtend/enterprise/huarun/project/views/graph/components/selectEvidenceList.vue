      ...
      <div class="layoutInnerAbsoluteDiv">
          ...
          <eco-content top="0px" height="60px" type="tool">
              <el-row style="padding:12px 10px;background-color:#fff;">
                    <el-col :span="8" >
                        <eco-tool-title style="line-height: 34px;" :title="'凭证模板（'+params.total+'）'"></eco-tool-title>
                    </el-col>
                ...
                >
                  ...
                  <el-table-column
                   ...
                   <el-table-column
                      ...
                      >
                      <template slot-scope="scope">
                            {{getKVName(scope.row.roleTypes,'crp_role_type')}}
                      </template>
                  </el-table-column>
...
<script>
import {getEvidenceList,delEvidenceAjax,getEnumSelectEnabled} from '../../../service/service.js'
    ...
    methods: {
        ...
        listAction(){
                let callBackDialogFunc = function(obj){
                    if(obj && obj.action == 'upEvidenceAddBack'){
        ...
        getListFunc(){
              getEvidenceList(this.params).then((response)=>{
        ...
        goAddPage(){
            if(sysEnv == 1){
                let url = '/project/index.html#/evidenceAdd';
                EcoUtil.getSysvm().openDialog('新增凭证',url,500,300,'8vh');
            }else{
                this.$router.push({name:'evidenceAdd'});
            }
        },

        editItem(id){
               if(sysEnv == 1){
                  let url = '/project/index.html#/evidenceEdit/'+id;
                  EcoUtil.getSysvm().openDialog('编辑凭证',url,500,300,'8vh');
              }else{
                  this.$router.push({name:'evidenceEdit',params:{id:id}});
              }
          },
          ...
          deleteItem(id){
              ...
              let confirmYesFunc = function(){
                  delEvidenceAjax(id).then((response)=>{
                      that.$message({
                              showClose: true,
                              message:'删除成功',
                              type: 'success',
          ...
          refreshFunc(){
                ...
                this.searchParams.business = null;
                this.params.page = 1;
                this.searchListFunc();
          },
