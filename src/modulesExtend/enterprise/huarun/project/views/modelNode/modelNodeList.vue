      ...
      <div class="layoutInnerAbsoluteDiv">
          ...
          <eco-content top="0px" height="60px" type="tool">
              <el-row style="padding:12px 10px;background-color:#fff;">
                    <el-col :span="8" >
                        <eco-tool-title style="line-height: 34px;" :title="'项目节点模板（'+params.total+'）'"></eco-tool-title>
                    </el-col>
          ...
          <eco-content top="59px" v-show="this.searchShow">
              <div class="searchBox">
                  ...
                  <span>模板名称：</span>
                  <div class="itemInput">
                      <el-input  placeholder="请输入模板名称" v-model="searchParams.name" @keyup.enter.native="searchListFunc"></el-input>
                  </div>
...
<script>
import {getModelNodeList,delModelNode} from '../../service/service.js'
    ...
    methods: {
        ...
        listAction(){
                let callBackDialogFunc = function(obj){
                    if(obj && obj.action == 'upModelNodeAddBack'){
        ...
        getListFunc(){
              getModelNodeList(this.params).then((response)=>{
        ...
        goAddPage(){
            if(sysEnv == 1){
                let url = '/project/index.html#/modelNodeAdd';
                EcoUtil.getSysvm().openDialog('新增节点模板',url,500,300,'16vh');
            }else{
                this.$router.push({name:'modelNodeAdd'});
            }
        },

        editItem(id){
               if(sysEnv == 1){
                  let url = '/project/index.html#/modelNodeEdit/'+id;
                  EcoUtil.getSysvm().openDialog('编辑节点模板',url,500,300,'16vh');
              }else{
                  this.$router.push({name:'modelNodeEdit',params:{id:id}});
              }
        },
