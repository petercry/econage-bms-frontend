...
<div class="filePower" v-show="form!=''">
    ...
    <el-form label-width="80px" :model="form">
        <el-form-item label="查看用户">
            <tag-select style="width:400px;" :initDataStr="exposeMembers" ref="tagSelect" :initOptions="{selectNum: 2,selectType: 'Dept-User-Role-userGroup'}" @callBack="exposeMember"></tag-select>
        </el-form-item>
        ...
        <el-form-item label="隐藏用户">
            <tag-select style="width:400px;vertical-align: top;" :initDataStr="hideMembers" ref="tagSelect" :initOptions="{selectNum: 2,selectType: 'Dept-User-Role-userGroup'}" @callBack="hideMember"></tag-select>
        </el-form-item>
        <el-form-item label="管理用户">
            <tag-select style="width:400px;vertical-align: top;" :initDataStr="manageMembers" ref="tagSelect" :initOptions="{selectNum: 2,selectType: 'Dept-User-Role-userGroup'}" @callBack="manageMember"></tag-select>
        </el-form-item>
...
export default {
    ...
    methods: {
        ...
        getFileData() {
            ...
            this.handleSelect(this.data.entity.cooperateLookupMembers, cooperateLookup)
            this.handleSelect(this.data.entity.cooperateDownloadMembers, cooperateDownload)
            let exposeStr = exposeArr.join('|')
            let hideStr = hideArr.join('|')
            let manageStr = manageArr.join('|')
            let cooperateLookupStr = cooperateLookup.join('|')
            let cooperateDownloadStr = cooperateDownload.join('|')

            this.exposeMembers = exposeStr
            this.hideMembers = hideStr
            this.manageMembers = manageStr
            this.cooperateLookupMembers = cooperateLookupStr
            this.cooperateDownloadMembers = cooperateDownloadStr
            // // 文件
