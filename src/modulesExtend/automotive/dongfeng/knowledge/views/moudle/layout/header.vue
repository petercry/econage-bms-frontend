...
<div class="kn-header">
    <el-row style="padding:12px 10px;background-color:#fff;">
        <el-col :span="24">
            <!-- <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle"></eco-tool-title> -->
            <!-- <el-input v-model="keyWords" type="text" style="width:200px;marginLeft:10px;verticalAlign:middle" placeholder="搜索名称">
                <i class="el-icon-search el-input__icon" slot="suffix" style="cursor:pointer"></i>
            </el-input> -->
            <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="type==4" :title="'企业标准'+''"></eco-tool-title>
            <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="type==2" :title="'外来标准'+''"></eco-tool-title>
            <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="type==3" :title="'业务指南'+''"></eco-tool-title>
            <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="type==1" :title="'通用文档'+''"></eco-tool-title>
            <el-button type="primary" size="small" class="left-float" @click.native="editFolder">修改当前文件夹</el-button>
            ...
            <el-button size="small" type="primary" class="left-float" @click.native="delMulti">删除</el-button>
        </el-col>
    </el-row>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        delMulti() {
            ...
            let ids1 = ids.join('&')
            let ids2 = delItems.map(item => {
                return item.id
            })
            let options = { type: 'warning', lockScroll: false }
            let confirmYesFunc = () => {
                // console.log(ids1, this.id)
                deleteFolderOrFile(ids1, this.id).then(res => {
                    // console.log(res)
        ...
        editFolder() {
            ...
            if (this.activeId == 'D.1.commonStd' || this.activeId == 'D.2.externalStd' || this.activeId == 'D.3.guideRevision' || this.activeId == 'D.4.stdRevision') {
                return ''
            } else {
                if (sysEnv !== 1) {
                    this.$router.push({ name: 'folderEdit', params: { id: this.activeId } })
                } else {
                    let url = '/knowledge/index.html#/folderEdit' + '/' + this.activeId;
