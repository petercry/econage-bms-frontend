            ...
            <el-form ref="form" :model="baseInfo" label-width="150px" label-position="right" style="padding-right:20px;" size="small">
                    ...
                    <el-form-item label="部门" prop="deptId" :rules="[{required: true, message:'部门必须填写',trigger: 'change'}]">
                            <tag-select 
                                ...
                                @callBack="selectDept"
                                :initDataStr="baseInfo.deptInitDataStr" >
                            </tag-select>
                    </el-form-item>  
                  
                    <el-form-item label="专业名称" prop="profession" :rules="[{required: !flag, message:'专业名称为必选项',trigger: 'change'}]">
                           <el-select
                                    style="width:100%"
...
import {addProMember,getUserInfoByOrgId} from '../../service/service'
...
import { mapState,mapActions } from 'vuex';
  ...
  export default {
      ...
      created(){
            ...
            this.baseInfo.projectId = this.$route.params.proId;
        ...
        initDept(data){
            ...
            getUserInfoByOrgId(data.itemArray[0].linkId).then(res => {
                ...
                if (!this.baseInfo.deptId) {
                    this.baseInfo.deptId = res.data.departments[len].id;//部门
                    this.baseInfo.deptInitDataStr = `{"type":"DEPT","orgId":"${this.baseInfo.deptId}","linkId":"${this.baseInfo.deptId}"}`;
                }
            })
        ...
        createFunc(){
                ...
                this.$refs['form'].validate((valid) => {
                     if (valid) {
                        ...
                        addProMember(that.baseInfo).then((response)=>{
                                that.$nextTick(() => { // 以服务的方式调用的 Loading 需要异步关闭
                                    loadingInstance.close();
                                });
                                if(response.data){
                                    let doObj = {}
                                    doObj.action = 'proMemberAddCallBack';
                                    doObj.data = {};
                                    doObj.close = true;
