<template>
  <div class="taskList">
    <div class="toolbar">
      <el-button type="primary" @click="Issue">同意</el-button>
    ...
    <eco-content :top="searchHeight" ref="content" bottom="42px" style="padding:0px 15px;">
      <el-table border stripe height="100%" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" class="styleTableDefaulta">
        ...
        <el-table-column prop="regulatoryCompliance" label="操作" width="60">
          <template slot-scope="scope">
            <span class="detailSpan" @click="Detail(scope.row.id)">办理</span>
          </template>
        </el-table-column>
      </el-table>
    </eco-content>
...
export default {
  ...
  computed:{
    ...mapState(['typeList','designStatusList'])
  },
  data() {
    return {
      searchform: {
        phase: "deptContactApproval",
        projectId: "",
        status: "",
        importantType: "",
        node: "",
  ...
  methods: {
    ...
    Detail(id) {
      if (sysEnv == 1) {
        let url =
          "/project/index.html#/handleStripesExamine/" + id + '/' + this.searchform.phase + '/' + this.projectId
        EcoUtil.getSysvm().openDialog("编辑任务", url, 1540, 500, "12vh");
      } else {
        this.$router.push({
