    ...
    <eco-content top="0px" bottom="0px" class="addBaseInfo">
        ...
        <eco-content top="0px" bottom="50px" >
            <el-form ref="form" :model="baseInfo" label-width="150px" label-position="right" style="padding-right:20px;">
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="项目类别" prop="category" :rules="[{required: true, message:'请选择项目类别',trigger: 'change'}]"  >
                                <el-select
                                    style="width:100%"
                    ...
                    <el-col :span="12">
                        <el-form-item label="预计EOP时间" prop="eopTime" :rules="[{ttype: 'date', required: true, message:'请选择预计EOP时间',trigger: 'change'}]">
                    ...
                    <el-col :span="24">
                        <el-form-item label="车辆类型" prop="carModelItems" :rules="[{type: 'array',required: true, message:'请至少选择一个车辆类型',trigger: 'change'}]">
                ...
                <el-row>
                    ...
                    <el-col :span="24">
                        <el-form-item label="商品/平台/发动机预概念" >
                             <ecoFileUploadChunk
...
import {getItemFetchId,addProBaseInfo} from '../../service/service'
...
import { mapState,mapActions } from 'vuex';
  ...
  export default {
      ...
      methods: {
        ...
        init(){
            this.getItemFetchIdFunc();
            this.initProjectBaseData('create-enabled').then(() => { });
        },

        //获取项目id
        getItemFetchIdFunc(){
            getItemFetchId().then((response)=>{
                this.baseInfo.id = response.data;
            })
        },
        ...
        createFunc(){
                ...
                this.$refs['form'].validate((valid) => {
                     if (valid) {
                        ...
                        addProBaseInfo(that.baseInfo).then((response)=>{
                                that.$nextTick(() => { // 以服务的方式调用的 Loading 需要异步关闭
                                    loadingInstance.close();
                                });
                                if(response.data){
                                        let doObj = {}
                                        doObj.action = 'proBaseInfoAddCallBack';
                                        doObj.data = {};
                                        doObj.data.entity = response.data;
                                        doObj.close = true;
