...
<eco-content top="0px" bottom="0px" type="tool" class="wfToDoVue" style="background-color:#f5f5f5">
    <div class="knowLedgeList">
        ...
        <eco-content top="0px" height="60px" type="tool" style="border-bottom:1px solid #ddd;">
            <el-row style="padding:12px 10px;background-color:#fff;">
                <el-col :span="24">
                    <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="params.category==4" :title="'企业标准'+''"></eco-tool-title>
                    <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="params.category==2" :title="'外来标准'+''"></eco-tool-title>
                    <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="params.category==3" :title="'业务指南'+''"></eco-tool-title>
                    <eco-tool-title style="line-height: 34px;margin-right:50px;fontWeight:700;verticalAlign:middle" v-show="params.category==1" :title="'通用文档'+''"></eco-tool-title>
                    ...
                    <el-input v-model="srchTxt" type="text" style="width:200px;marginLeft:10px;verticalAlign:middle" placeholder="搜索知识库名称" @keyup.enter.native="handleSearchClick"> <i class="el-icon-search el-input__icon" slot="suffix" style="cursor:pointer" @click="handleSearchClick">
                        </i></el-input>
                    <el-button type="primary" class="toolBtn" @click.native="addNewKnowledgeLib"><i class="el-icon-document-add" style="marginRight:4px"></i>&nbsp;新建知识库</el-button>
                </el-col>
            </el-row>
        </eco-content>
        <eco-content bottom="0" top="61px" id="contentList" style="padding:10px 15px;">
                        ...
                        <el-card class="box-card" shadow="never" @click.native="goDetail(item)">
                            <div slot="header" class="clearfix">
                                ...
                                <el-popover placement="right" width="20" trigger="hover">
                                    <div style="cursor:pointer" class="card-edit" @click="goEdit(item.id)">编辑</div>
                                    <div style="cursor:pointer" class="card-delete" @click="deleteCurrent(item.id)">删除</div>
                                    <i class="el-icon-more" style="float: right; padding: 3px 0;transform: rotate(90deg);fontSize:18px;color:#bcbcbc;cursor:pointer" slot="reference"></i>
                                </el-popover>
                            </div>
...
export default {
    ...
    created() {
      // console.log(this.$route,'index')
        if (this.$route.params) {
            ...
            this.params.category = this.$route.params.type
            // this.SET_TYPE(this.params.category)
    ...
    methods: {
        ...mapActions([]),
        ...mapMutations(['SET_FILETREENODE', 'SET_ACTIVEID','SET_TYPE']),
        // 添加监听
        addMonitor() {
        ...
        addNewKnowledgeLib() {
            if (sysEnv !== 1) {
                ...
                EcoUtil.getSysvm().openDialog('新建知识库', url, 650, 550, '12vh');
            }
        },
        goEdit(id) {
            if (sysEnv !== 1) {
                ...
                EcoUtil.getSysvm().openDialog('编辑知识库', url, 650, 550, '12vh');
            }
        },
        goDetail(item) {
            this.SET_TYPE(this.params.category)
            if (sysEnv !== 1) {
                this.$router.push({ name: 'knowLibDetail', params: { id: item.id,type:this.params.category } })
            } else {
                let tabObj = {};
                tabObj.desc = item.name
                let goPage = "knowledge/index.html#/knowLibDetail" + '/' + item.id+'/'+this.params.category;
