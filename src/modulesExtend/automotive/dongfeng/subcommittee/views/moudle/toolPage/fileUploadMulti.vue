<template>
<div class="fileUploadMulti" v-loading="loading">
    <!-- 批量上传 -->
    <div>
        <!-- <el-form label-width="70px" class="demo-ruleForm" label-position="left" :model="form" :rules="rules" ref="ruleForm">
            <el-form-item label="文件" prop="file">
                <eco-file-upload-btn :modular="'knowledge'" :modularInnerId="modularInnerId" @fileChange="fileChange" style="width:400px;display:inline-block;" @fileOnSuccess="fileOnSuccess" ref="fileUpload"></eco-file-upload-btn>
                <div style="display:inline-block;width:100%">
                    <p v-for="(item,index) in fileList" :key="item.id" class="fileBox">
                        <span class="ellipsis" style="max-width:300px">{{item.name}}</span>
                        <i class="el-icon-close" style="float:right;line-height:36px;cursor:pointer" @click="deleteFile(item,index)"></i>
                    </p>
                </div>
            </el-form-item>
