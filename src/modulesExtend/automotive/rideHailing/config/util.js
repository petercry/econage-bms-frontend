...
export default {
  ...
  restData(data, type) {
    ...
    if(type==='status'){
      for (var key in this.$store.state.statusMenu) {
        if (data == key) {
          str = this.$store.state.statusMenu[key]
        }
      }
    }else if(type==='operate'){
      for (var key in this.$store.state.operateMenu) {
        if (data == key) {
          str = this.$store.state.operateMenu[key]
        }
      }
    }
    return str;
