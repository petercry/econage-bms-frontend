import {getTreeSelectEnabled,getEnumSelectEnabled,regulationtrackStatusSet} from '../service/service.js'
import {
    SET_TYPE_LIST,
    SET_SUBCLASS_LIST,
    SET_NATURE_LIST,
...
export default {
    ...
    setRegulationVersion({commit},action='create-enabled'){
        //法规版本
        return new Promise((resolve,reject)=>{
            getEnumSelectEnabled('1120',action).then(res=>{
                commit(SET_REGULATION_VERSION,res.data);
                resolve();
            }).catch(err=>{
                reject(err);
            })
