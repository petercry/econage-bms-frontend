        ...
        <div class="ecoFileUploadBtn">
          ...
          <ul class="el-upload-list el-upload-list--text" v-if="showList">
            <li class="el-upload-list__item is-success" v-for="(item,index) in fileList" :key="item.id" :class='{"mT0":(index==0 && !isEdit)}'>
              ...
              <a target="_blank" class="download-btn" v-if="item.bodyId && isShowDownBtn" :href="formatUrlWithAuthToken(options.webFileSingleDownloadUrl+'/'+item.id)">
                <i class="el-icon-download"></i>
              </a>
              <a class="preview-btn" :style="`right:${isShowDownBtn?'44':'20'}px`" @click.prev.stop='preView(item)' v-if="item.bodyId&&options.previewOfficeFile&&_isPreviewFile(item.fileType)" href="javascript:;">
