      ...
      <eco-content top="56px" bottom="0px" type="tool">
        ...
        <div class="contentRight">
          <el-col style="margin-top:40px">
            ...
            <el-row span="24" class="flex">
              <span class="leftText">序号</span>
              <span class="inputBox">
                <el-input :disabled='!buttonStatus' v-model="params.order" size="mini"></el-input>
                <!-- <el-input v-else disabled v-model="params.name" size="mini"></el-input> -->
              </span>
            </el-row>
            <!-- <el-row span="24" class="flex">
...
import { technicalStandardTree,getTechnicalTable,delTechnicalTable,addTechnicalTable,updataTechnicalTable } from "../service/service.js";
...
export default {
  ...
  methods: {
    ...
    saveCase() {
      if(this.addStatus) {
        addTechnicalTable(this.params).then(res => {
          this.$message.success("保存成功")
          // this.addStatus = false
          this.reloadNode()
        })
      } else {
        updataTechnicalTable(this.params).then(res => {
    ...
    editNode() {
      ...
      this.params.order = this.currentNode.order
      // updataTechnicalTable(this.params).then(res => {
      //   this.$message.success("保存成功")
      // })
    },
    ...
    delNode() {
      ...
      delTechnicalTable(this.currentNode.id).then(res => {
    ...
    handleNodeClick(data) {
      ...
      getTechnicalTable(data.id).then(res => {
        console.log(res.data)
        this.params.parentName = res.data.parentName
        this.params.code = res.data.code
        this.params.name = res.data.name
        this.params.order = res.data.order
