<template>
    <div class='editWorkingGroup'>
        <div class='addForm'>
            <el-form :model='formData' ref='managementForm' :rules='rules' v-loading='loading' label-position='right' label-width='110px'>
                <el-row>
                    <el-col :span='8'>
                ...
                <el-row>
                    <el-col :span='8'>
                        <el-form-item label='TC号:' prop='tcNum'>
                            <el-input v-if='isEdit' placeholder='请输入TC号' v-model='formData.tcNum'></el-input>
                            <span v-else class='viewContent'>{{formData.tcNum}}</span>
                        </el-form-item>
                    </el-col>
                    <el-col :span='8'>
                        <el-form-item label='SC号:' prop='scNum'>
                            <el-input v-if='isEdit' placeholder='请输入SC号' v-model='formData.scNum'></el-input>
                            <span v-else class='viewContent'>{{formData.scNum}}</span>
                        </el-form-item>
                    </el-col>
                    <el-col :span='8'>
                        <el-form-item label='工作组全称:' prop='workGroupName'>
                            <el-input v-if='isEdit' placeholder='请输入工作组全称' v-model='formData.workGroupName'></el-input>
                            <span v-else class='viewContent'>{{formData.workGroupName}}</span>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span='8'>
                        <el-form-item label='工作组联系人:' prop='workGroupContact'>
                            <el-input v-if='isEdit' placeholder='请输入工作组联系人' v-model='formData.workGroupContact'></el-input>
                            <span v-else class='viewContent'>{{formData.workGroupContact}}</span>
                        </el-form-item>
                    </el-col>
                    <el-col :span='8'>
                        <el-form-item label='联系方式:' prop='workGroupPhone'>
                            <el-input v-if='isEdit' placeholder='请输入联系方式' v-model='formData.workGroupPhone'></el-input>
                            <span v-else class='viewContent'>{{formData.workGroupPhone}}</span>
                        </el-form-item>
                    </el-col>
                    <el-col :span='8'>
                        <el-form-item label='Email:' prop='workGroupEmail'>
                            <el-input v-if='isEdit' placeholder='请输入Email' v-model='formData.workGroupEmail'></el-input>
                            <span v-else class='viewContent'>{{formData.workGroupEmail}}</span>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
            ...
            <el-table class='infoTable' :data='infoList' border style='width:100%;' :header-cell-style="{background:'#f5f7fa',color:'#000',textAlign:'center'}"
                ...
                <el-table-column label='参加人姓名' prop='employee' width='150'>
                ...
                <el-table-column label='单位' prop='unit' min-width='170'>
                    <template slot-scope='scope'>
                ...
                <el-table-column label='部门' prop='dept' min-width='170'>
                    <template slot-scope='scope'>
                ...
                <el-table-column label='职务/职称' prop='position' width='150'>
                    <template slot-scope='scope'>
                ...
                <el-table-column label='座机' prop='telephone' width='150'>
                    <template slot-scope='scope'>
                        <el-input v-if='isEdit' v-model='scope.row.telephone'></el-input>
                        <span v-else>{{scope.row.telephone}}</span>
                    </template>
                </el-table-column>
                <el-table-column label='手机' prop='mobilePhone' width='150'>
                    <template slot-scope='scope'>
...
<script>
    ...
    import { workGroupAddSingle, getUserInfoByOrgId,workGroupSelectById,workGroupUpdateSingle } from '../../service/service.js'
    ...
    export default {
        ...
        data() {
            var validateTelephone = (rule, value, callback) => {
                if (rule.required) {
                    //电话
                    if (!value) {
                        callback(new Error('工作组联系方式为必填项'));
            ...
            return {
                ...
                rules: {
                    ...
                    'workGroupName': [{ required: true, message: '工作组全称为必填项', trigger: 'blur' }],
                    'workGroupContact': [{ required: true, message: '工作组联系人为必填项', trigger: 'blur' }],
                    'workGroupPhone': [{ required: true, validator: validateTelephone, trigger: 'blur' }],
                    'workGroupEmail': [{ required: true, message: 'Email为必填项', trigger: 'blur' },{ type: 'email', message: '邮箱格式不正确', trigger: 'blur' }]
                },
                loading: false,
                id: '',
                caseType:''
        ...
        methods: {
            getDetailInfo(){
                this.loading=true;
                workGroupSelectById(this.id).then(res=>{
                ...
                this.$refs.managementForm.validate((valid) => {
                    if (valid) {
                        ...
                        var params={
                            ...
                            workGroupName: this.formData.workGroupName,
                            workGroupContact: this.formData.workGroupContact,
                            workGroupPhone: this.formData.workGroupPhone,
                            workGroupEmail: this.formData.workGroupEmail,
                            staffList:[]
                        }
                        ...
                        if (this.id == 0) {
                            //新增
                            workGroupAddSingle(params).then(res => {
                                this.loading = false;
                                let doObj = {}
                                doObj.action = 'addWorkingGroup';
                                doObj.data = {};
                                doObj.close = true;
                            ...
                            params.id=this.id;
                            workGroupUpdateSingle(params).then(res=>{
                                this.loading=false;
                                let doObj = {}
                                doObj.action = 'editWorkingGroup';
                                doObj.data = {};
                                doObj.close = true;
                                EcoUtil.getSysvm().callBackDialogFunc(doObj);
                            }).catch(err=>{
...
<style scoped>
    ...
    .editWorkingGroup .addForm .el-input,
    .editWorkingGroup .addForm .el-select {
        width: 200px;
    }

    .editWorkingGroup .infoTable .el-input {
        width: 90%;
    }

    .editWorkingGroup .caseBtn {
        margin-top: 10px;
