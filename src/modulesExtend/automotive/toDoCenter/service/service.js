...
//待办中心列表
export const taskAggregationList = function(){
  return axios.get(baseUrl+'/dongfeng/task-aggregation/list',{
    params:{
      time:new Date().getTime()
    }
  })
...
//查询项目id
export const projectListSelectId = function(id){
  return axios.get(baseUrl+'/dongfeng/thinktank/project/list/select-pending',{
    params:{
      id,
      time:new Date().getTime()
    }
...
//查询法规点检
export const dfHandlingInterfaceSendMap  = function(stepStatus){
  return axios.get(baseUrl+'/dongfeng/productioncar/dfHandlingInterface/sendMap',{
    params:{
      stepStatus,
      time:new Date().getTime()
    }
