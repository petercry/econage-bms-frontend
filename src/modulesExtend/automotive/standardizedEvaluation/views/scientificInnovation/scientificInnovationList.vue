    ...
    <div class='certificate'>
      ...
      <eco-content top='0px' type='tool'>
        <el-row style='padding: 14px;background:#fff;border: 1px solid #ddd;'>
          <el-col :span='5' style='height:30px;line-height: 30px;'>
            <eco-tool-title title='科创负责人确认'></eco-tool-title>
          </el-col>
          <el-col :span='19' style='text-align:right'>
            <!-- class="plainBtn"  -->
            <el-button type='primary' size='small' @click='changeSearchShow'>高级查询</el-button>
            <el-button type='primary' size='small' @click='rejectionCase'>同意</el-button>
            <el-button type='primary' size='small' @click='rejectionCase'>驳回</el-button>
          </el-col>
        </el-row>
      </eco-content>
      <eco-content v-show='isShowSearch' top='59px' type='tool' style='border:1px solid #ddd;overflow: hidden;'>
        <el-row style="padding:15px 10px 16px 10px;background:#fff">
          ...
          <el-col :span="24" style="margin-top:7px">
            <span class='searchInputLabel'>制订人:</span>
            <el-input clearable style='width:150px' @keyup.enter.native="requestData('search')" v-model='searchContent.publisher'
              placeholder='请输入'>
              <i class='el-icon-search el-input__icon' slot='suffix'></i>
            </el-input>
            &emsp;
            <el-button @click='requestData("search")' type='primary'>查询</el-button>
            <el-button @click='restSearContent'>重置</el-button>
          </el-col>
          <el-col>
              <div style="color:red;margin:15px 0px 30px 0px;font-size:14px;">注：2020年度应到20条，实到10条，需等待所有都到达后，才可提交审批。</div>
          </el-col>
        </el-row>
      </eco-content>
      <eco-content :top='contentTop' bottom='42px' style='padding:10px 15px;border:1px solid #ddd;'>
        <el-table ref='certificateListTable' stripe :data='tableData' header-row-class-name='tableHeader' border tooltip-effect='dark'
          ...
          <el-table-column label='状态' prop='time'></el-table-column>
          <el-table-column label='年度' prop='time'></el-table-column>
          <el-table-column label='部门' prop='time'></el-table-column>
          <el-table-column label='标准分类' prop='time'></el-table-column>
          <el-table-column label='标准类型' prop='time'></el-table-column>
          <el-table-column label='标准编号' prop='time'>
              <template slot-scope="scope">
                  <span @click="toView(scope.row)">{{scope.row.time}}</span>
              </template>
          </el-table-column>
          <el-table-column label='标准名称' prop='time'></el-table-column>
          <el-table-column label='发布时间' prop='time'></el-table-column>
          <el-table-column label='实施时间' prop='time'></el-table-column>
          <el-table-column label='科室' prop='time'></el-table-column>
          <el-table-column label='制定人' prop='time'></el-table-column>
          <el-table-column label='分标委' prop='time'></el-table-column>
        </el-table>
      </eco-content>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState(['typeList']),
      contentTop() {
        return this.isShowSearch ? '199px' : '59px';
      }
    },
    data() {
      return {
        ...
        tableData: [
            {year: '2019',user: 'xyd',time:"2011-9-9",isView:true},
            {year: '2019',user: 'xyd',time:"2011-9-9",isView:true},
            {year: '2019',user: 'xyd',time:"2011-9-9",isView:false}
        ],
        multipleSelection: [],
        typeData: [],
        statusData: [],
    ...
    methods: {
      ...
      rejectionCase() {
        if (sysEnv==0){
            this.$router.push({name:'rejectionDescription',params:{id: '1'}})
        }else{
            EcoUtil.getSysvm().openDialog('新增评比','/standardizedEvaluation/#/rejectionDescription/' + '1',650,350,'20vh');
        }
      },
      //重置输入框
      restSearContent() {
      ...
      toView(data) {
        if (sysEnv==0){
            if(data.isView == false) {
                this.$router.push({name:'declarantFeedbackAdd',params: {id: data.id}})
            } else {
                this.$router.push({name:'declarantFeedbackView',params: {id: data.id}})
            }
        }else{
            let goPage = null
            if(data.isView == false) {
                goPage = "standardizedEvaluation/index.html#/declarantFeedbackAdd/" + data.id;
            } else {
                goPage = "standardizedEvaluation/index.html#/declarantFeedbackView/" + data.id;
            }
            let tabObj = {};
