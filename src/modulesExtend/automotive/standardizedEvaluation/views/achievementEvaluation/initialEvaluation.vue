    ...
    <div class='certificate'>
      ...
      <eco-content top='0px' type='tool'>
        <el-row style='padding: 14px;background:#fff;border: 1px solid #ddd;'>
          <el-col :span='5' style='height:30px;line-height: 30px;'>
            <eco-tool-title title='初评'></eco-tool-title>
          </el-col>
          <el-col :span='19' style='text-align:right'>
            <!-- class="plainBtn"  -->
            <el-button type='primary' size='small' @click='changeSearchShow'>高级查询</el-button>
            <el-button type='primary' size='small' @click='initCase'>初始化</el-button>
            <el-button type='primary' size='small' @click='addCase'>下发确认</el-button>
            <el-button type='primary' size='small' @click='addCase'>新增</el-button>
            <el-button type='primary' size='small' @click='addCase'>删除</el-button>
            <el-button type='primary' size='small' @click='addCase'>导入</el-button>
            <el-button type='primary' size='small' @click='addCase'>导出</el-button>
          </el-col>
        </el-row>
      </eco-content>
      <eco-content v-show='isShowSearch' top='59px' type='tool' style='border:1px solid #ddd;overflow: hidden;'>
        <el-row style="padding:15px 10px 16px 10px;background:#fff">
          <el-col :span='24' style='margin-top:7px;'>
            <span class='searchInputLabel'>&emsp;是否推荐:</span>
            ...
            <span class='searchInputLabel'>&emsp;部门:</span>
          ...
          <el-col :span="24" style="margin-top:7px">
            <span class='searchInputLabel'>&emsp;&emsp;制订人:</span>
            <el-input clearable style='width:150px' @keyup.enter.native="requestData('search')" v-model='searchContent.publisher'
              placeholder='请输入'>
              <i class='el-icon-search el-input__icon' slot='suffix'></i>
            </el-input>
      ...
      <eco-content :top='contentTop' bottom='42px' style='padding:10px 15px;border:1px solid #ddd;'>
        <el-table ref='certificateListTable' stripe :data='tableData' header-row-class-name='tableHeader' border tooltip-effect='dark'
          ...
          <el-table-column label='是否推荐' prop='user'></el-table-column>
          ...
          <el-table-column label='标准编号' prop='user'>
            <template slot-scope="scope">
              <span @click="toInfo(scope.row.id)">{{scope.row.user}}</span>
            </template>
          </el-table-column>
          <el-table-column label='标准名称' prop='user'></el-table-column>
          <el-table-column label='发布时间' prop='user'></el-table-column>
          <el-table-column label='实施时间' prop='user'></el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        tableData: [
            {year: '2019',user: 'xyd',time:"2011-9-9",id: '13245135461231'},
    ...
    methods: {
      ...
      initCase() {
        if (sysEnv==0){
            this.$router.push({name:'createInitList'})
        }else{
            EcoUtil.getSysvm().openDialog('新增评比','/standardizedEvaluation/#/createInitList',400,150,'35vh');
        }
      },
      //搜索
      requestData() {
      ...
      addCase() {
        if (sysEnv==0){
            ...
            EcoUtil.getSysvm().openDialog('新增评比','/standardInformationRelease/#/standardAdd',800,700,'10vh');
        }
      },
      viewSent() {
      ...
      toInfo(data) {
        if (sysEnv==0){
            this.$router.push({name:'srandardInfo',params: {id: data.id}})
        }else{
          let tabObj = {};
          let goPage = "standardizedEvaluation/index.html#/srandardInfo/" + data.id;
