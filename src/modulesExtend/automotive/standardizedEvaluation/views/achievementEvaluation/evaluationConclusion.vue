      ...
      <eco-content top='0px' type='tool'>
        <el-row style='padding: 14px;background:#fff;border: 1px solid #ddd;'>
          <el-col :span='5' style='height:30px;line-height: 30px;'>
            <eco-tool-title title='盲评及评比结论'></eco-tool-title>
          </el-col>
          <el-col :span='19' style='text-align:right'>
            <!-- class="plainBtn"  -->
            <el-button type='primary' size='small' @click='changeSearchShow'>高级查询</el-button>
            <el-button type='primary' size='small' @click='groupCase'>分组</el-button>
            <el-button type='primary' size='small' @click='addCase'>下发</el-button>
            <el-button type='primary' size='small' @click='addCase'>新增</el-button>
            <el-button type='primary' size='small' @click='addCase'>删除</el-button>
            <el-button type='primary' size='small' @click='getResult'>获奖结果</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getResult() {
        if (sysEnv==0){
            this.$router.push({name:'getResultAdd',params:{id: '1'}})
        }else{
            EcoUtil.getSysvm().openDialog('新增评比','/standardizedEvaluation/#/getResultAdd/' + '1',800,700,'10vh');
        }
      },
      addCase() {
        if (sysEnv==0){
      ...
      groupCase() {
        if (sysEnv==0){
            this.$router.push({name:'selectGroup',params:{id: '1'}})
        }else{
            EcoUtil.getSysvm().openDialog('新增评比','/standardizedEvaluation/#/selectGroup/' + '1',400,150,'35vh');
        }
      },
      viewSent() {
