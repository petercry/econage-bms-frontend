<template>
  <div class="officeConfim" v-loading="loading">
    <div class="clearfix">
      <div class="btn">
  ...
  methods: {
    ...
    deit(val) {
      if (sysEnv !== 1) {
        this.$router.push({
          name: "conditionEdit",
          params: { id: val.id, name: 'OFFICE_LIAISON_CHOOSE' },
        });
      } else {
        let _url = "/standardPlanning/index.html#/conditionEdit/" + val.id + '/' + 'OFFICE_LIAISON_CHOOSE';
    ...
    // 添加
    add() {
      // let _url = "/standardPlanning/index.html#/addStandard/OFFICE_LIAISON_CHOOSE";
      // EcoUtil.getSysvm().openDialog("新增", _url, "800", "700", "8hv");
      if (sysEnv !== 1) {
        this.$router.push({
          name: "addStandard",
          params: { phaseId: 'OFFICE_LIAISON_CHOOSE' },
        });
      } else {
        let _url = "/standardPlanning/index.html#/addStandard/OFFICE_LIAISON_CHOOSE";
        EcoUtil.getSysvm().openDialog("新增", _url, "800", "700", "8hv");
    ...
    listAction() {
      let callBackDialogFunc = function (obj) {
        if (
          ...
          window.officeConfimvm.getList();
        }
      };
      EcoUtil.addCallBackDialogFunc(callBackDialogFunc, "officeConfimvm");
    },
  },
};
</script>
...
<style>
.officeConfim .el-button--primary.is-plain {
...
.officeConfim .el-form-item--mini.el-form-item,
.el-form-item--small.el-form-item {
  margin-bottom: 10px;
}
.officeConfim .export:focus {
  outline: none;
  box-shadow: none;
  background-color: transparent;
}
.officeConfim .el-pagination {
  float: right;
