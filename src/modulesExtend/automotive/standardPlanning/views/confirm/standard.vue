  ...
  <div class="standard" v-loading="loading">
    <div class="clearfix">
      <div class="btn">
        ...
        <el-upload :action="wageaddress" accept=".xlsx" :headers="headers" :file-list="fileList" :on-success="upLoadSuc" :on-error="upLoadErr" :on-change="getList" style="display: inline-block;" :show-file-list=false>
        ...
        <el-button type="primary" @click="startUp">启动规划</el-button>
      </div>
    </div>
    <!-- 全展开搜索区域 -->
        ...
        <el-row v-show="all == 'open'">
           <el-col :span="6">
            <el-form-item label="审批节点">
              <el-select v-model="form.phaseId" placeholder="请选择" style="width:100%">
    ...
    <div class="table">
      <el-table :data="tableData" ref="multipleTable" style="width: 100%" height="100%" stripe :header-cell-style="{background:'##f5f7fa'}" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="分标委">
          <template slot-scope="scope">
            <span>{{scope.row.subcommitteeName}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" width="300">
          <template slot-scope="scope">
            <el-button type="text" size="small" v-show="scope.row.editStatus" @click="deit(scope.row)">修改</el-button>
...
export default {
  ...
  methods: {
    ...
    // getEnabList(e) {
    //   getEnabList(e).then(res => res.data)
    // },
    //获取列表数据
    getList() {
          //获取分标委列表
      getSubList().then(ress => {
        // this.subcommitteeList = ress.data.rows
             getStandardList(this.form).then((res) => {
                res.data.rows.forEach(x => {
                  getEnabList(x.classification).then(response => {
                    // x.typeListName = response.data
                    x.typeName = response.data.find(a => x.type == a.id)
                  })
                  //判断是否在该环节下可以修改
                  if (x.phaseId == 'TECH_INNOVATION_DEPT_CREATE') {
                    x.editStatus = true
                  } else {
                    x.editStatus = false
                  }
                ...
                //   systemCodeList(x.systemCode).then(res => {
                //       x.systemCode = res.data.name
                //   })
                // })
    ...
    deit(val) {
      if (sysEnv !== 1) {
        this.$router.push({
          name: "conditionEdit",
          params: { id: val.id, name: 'TECH_INNOVATION_DEPT_CREATE' },
        });
      } else {
        let _url = "/standardPlanning/index.html#/conditionEdit/" + val.id + '/' + 'TECH_INNOVATION_DEPT_CREATE';
    ...
    add() {
      if (sysEnv !== 1) {
        this.$router.push({
          name: "addStandard",
          params: { phaseId: 'TECH_INNOVATION_DEPT_CREATE' },
        });
      } else {
          let _url = "/standardPlanning/index.html#/addStandard/TECH_INNOVATION_DEPT_CREATE";
    ...
    detaliDpet() {
      ...
      if (this.selcetrows.length == 0) {
        ...
        return
          ...
          if (sysEnv !== 1) {
            this.$router.push({name: "detaliDpet",params: { ids: list.toString(),phaseId: 'TECH_INNOVATION_DEPT_CREATE' }});
          } else {
            let _url = "/standardPlanning/index.html#/detaliDpet/" + list.toString() + '/' + 'TECH_INNOVATION_DEPT_CREATE';
    ...
    issue() {
      // let stateArr = [];
      // for (let i of this.selcetrows.keys()) {
      //   if (this.selcetrows[i].phaseId) {
      //     stateArr.push(this.selcetrows[i].phaseId);
      //   }
      // }
      ...
      if(this.selcetrows && this.selcetrows.length != 0) {
        ...
        if (sysEnv !== 1) {
          this.$router.push({name: "selectDeptList",params: { ids: list.toString(), status: 'TECH_INNOVATION_DEPT_CREATE' },});
        } else {
          let _url = "/standardPlanning/index.html#/selectDeptList/" + list.toString() + '/' + 'TECH_INNOVATION_DEPT_CREATE';
          EcoUtil.getSysvm().openDialog("部门信息",_url,"800","700","8hv");
        }
      } else {
        this.$message.warning("请选择下发标准")
      }
      // issueAjax('TECH_INNOVATION_DEPT_CREATE').then((res) => {
      //   if (res.data.success) {
      //     this.$message({
      //       message: "下发成功",
      //       type: "success",
    ...
    upLoadSuc(response) {
      if (response.success) {
        ...
        EcoFile.openFileHeaderByDownload(response.fileHeaderId,response.fileName)
      } else {
    ...
    listAction() {
      let callBackDialogFunc = function (obj) {
        if (
          ...
          window.standardvm.getList();
        }
      };
      EcoUtil.addCallBackDialogFunc(callBackDialogFunc, "standardvm");
    },
  },
};
</script>
