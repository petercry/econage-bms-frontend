  ...
  data() {
    return {
      ...
      form: {
        ...
        sourceNumber: "",
        systemCode: "",
        phaseId: "BUSINESS_PLAN_DEPT_INITIATE",
        page: 1,
        rows: 10,
      },
      total: 0,
  ...
  methods: {
    ...
    deit(val) {
      if (sysEnv !== 1) {
        this.$router.push({
          name: "conditionEdit",
          params: { id: val.id, name: 'BUSINESS_PLAN_DEPT_INITIATE' },
        });
      } else {
        let _url = "/standardPlanning/index.html#/conditionEdit/" + val.id + '/' + 'BUSINESS_PLAN_DEPT_INITIATE';
