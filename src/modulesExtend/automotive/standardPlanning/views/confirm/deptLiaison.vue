<template>
  <div class="chiefReview" v-loading="loading">
    ...
    <!-- 表格 -->
    <div class="table">
      <el-table :data="topTable" ref="multipleTable" border style="width: 100%" height="20%" stripe :header-cell-style="{background:'##f5f7fa'}">
        <el-table-column v-for="(item,index) in tableHeader" :key="index" :prop="item.id" :label="item.name">
        </el-table-column>
      </el-table>
      <el-table :data="tableData" style="width: 100%" height="80%" stripe :header-cell-style="{background:'##f5f7fa'}" @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getDeptArrpTable(this.form.phaseId).then(res => {
        this.tableHeader = res.data.data.header
        this.topTable = res.data.data.data
