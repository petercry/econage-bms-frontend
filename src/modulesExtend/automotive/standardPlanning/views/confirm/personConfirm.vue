<template>
  <div class="personConfirm" v-loading="loading">
    <div class="clearfix">
      <div class="btn">
        ...
        <el-button class="export" @click="issue">提交</el-button>
  ...
  methods: {
    ...
    deit(val) {
      if (sysEnv !== 1) {
        this.$router.push({
          name: "conditionEdit",
          params: { id: val.id,name:'RESPONSIBLE_HANDLE' },
        });
      } else {
        let _url = "/standardPlanning/index.html#/conditionEdit/" + val.id + "/RESPONSIBLE_HANDLE";
    ...
    approval(val) {
      if (sysEnv !== 1) {
        ...
        let _url =
          "/standardPlanning/index.html#/projectList/" + val.id;
    ...
    review(val) {
      if (sysEnv !== 1) {
        ...
        let _url =
          "/standardPlanning/index.html#/ReexaminationForm/" +
          val.id;
    ...
    // 添加
    add() {
      // let _url = "/standardPlanning/index.html#/addStandard/RESPONSIBLE_HANDLE";
      // EcoUtil.getSysvm().openDialog("新增", _url, "800", "700", "8hv");
      if (sysEnv !== 1) {
        this.$router.push({
          name: "addStandard",
          params: { phaseId: 'RESPONSIBLE_HANDLE' },
        });
      } else {
        let _url = "/standardPlanning/index.html#/addStandard/RESPONSIBLE_HANDLE";
        EcoUtil.getSysvm().openDialog("新增", _url, "800", "700", "8hv");
    ...
    listAction() {
      let callBackDialogFunc = function (obj) {
        if (
          ...
          window.personConfirmvm.getList();
        }
      };
      EcoUtil.addCallBackDialogFunc(callBackDialogFunc, "personConfirmvm");
    },
  },
};
</script>
...
<style>
.personConfirm .el-button--primary.is-plain {
  background-color: white;
}
.personConfirm .export {
  color: #409eff;
  border: 1px solid #409eff;
}
.personConfirm .el-form-item--mini.el-form-item,
.el-form-item--small.el-form-item {
  margin-bottom: 10px;
}
.personConfirm .export:focus {
  outline: none;
  box-shadow: none;
  background-color: transparent;
}
.personConfirm .el-pagination {
  float: right;
