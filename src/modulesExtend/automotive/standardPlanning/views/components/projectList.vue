  ...
  <div class="projectList">
    <el-form ref="form" :model="form" label-width="130px" :rules="rules">
      ...
      <el-form-item label="适用范围、目的" prop="applicationScope">
        <el-input style=" margin-top: 10px;" type="textarea" :rows="4" placeholder="请输入内容" v-model="form.applicationScope" :readonly="onRead">
        </el-input>
      </el-form-item>
      <el-form-item label="标准的主要内容" prop="mainContent">
        <el-input style=" margin-top: 10px;" type="textarea" :rows="4" placeholder="请输入内容" v-model="form.mainContent" :readonly="onRead">
        </el-input>
      </el-form-item>
      <el-form-item label="需要解决的主要问题和补充实验，研究内容">
        <el-input style=" margin-top: 10px;" type="textarea" :rows="4" placeholder="请输入内容" v-model="form.mainProblem" :readonly="onRead">
        </el-input>
      </el-form-item>
      <el-form-item label="相关的国际标准、国外先进标准和国内相关标准（名称、编号）" prop="relatedStandard">
        <el-input style=" margin-top: 10px;" type="textarea" :rows="4" placeholder="请输入内容" v-model="form.relatedStandard" :readonly="onRead">
        </el-input>
      </el-form-item>
      <el-form-item label="对实际工作的指导作用" prop="guidingFunction">
        <el-input style=" margin-top: 10px;" type="textarea" :rows="4" placeholder="请输入内容" v-model="form.guidingFunction" :readonly="onRead">
        </el-input>
      </el-form-item>
      <el-form-item label="备注">
        <el-input style=" margin-top: 10px;" type="textarea" :rows="4" placeholder="请输入内容" v-model="form.remarks" :readonly="onRead">
        </el-input>
      </el-form-item>
      <el-form-item label="相关文档">
        <upload :isEdit='true' :showList='true' :multiple="false" :modular="modular" :modularInnerId="id" @fileChange="fileChange" @preView='preView' @fileOnSuccess="fileOnSuccess" accept='' @beforeFileUpload="beforeFileUpload">
          <el-button slot="uploadBtn" size="medium" class="uploadBtn">
            <i class="el-icon-plus"></i> 上传附件
          </el-button>
        </upload>
      </el-form-item>
      <el-form-item v-if="!onRead">
        <div class="btn">
  ...
  methods: {
    ...
    getform() {
      getProjectList(this.id).then((res) => {
        ...
        // //需要填写
        // if(!this.form.applicationScope && !this.form.mainContent && !this.form.mainProblem && !this.form.relatedStandard && !this.form.guidingFunction && !this.form.remarks) {
        //   this.onRead = false
        // } else {
        //   this.onRead = true
        // }
