<template>
  <div class="handleGridRadioVue designItem designGridItem">
            <el-radio-group v-model="value" size="mini" disabled="(!isEditable || isReadonly)" @change="onChangeEvent">
                      <el-radio 
       ...
       /*设置相关性*/
        ...
        getRefCheck(){ //检查 是否可以提交
            if(this.isEditable ){
                if(this.isRequired ){
                    if(!this.value || this.value == ''){
                        return {status:1,msg:this.mItem.itemName+' 必须选择(数据方正第'+(this.rowIdx+1)+'行，第'+(this.formIdx+1)+'列)',checkType:'RADIO',checkId:'handleItem_'+this.mItem.viewId+'_'+this.mItem.itemId+'_'+this.rowIdx+'_'+this.formIdx} ;
                    }
                }
            }
            return {status:0}
