<template>
    <div class="handleGridRelWFGrid designItem designGridItem" >
         <el-input 
                 :value="value"   
        ...
        ></el-input>
            ...
            <div v-else class="viewRelWfDiv" v-bind:class="{'iptReadonlyDiv':(!isEditable|| isReadonly)}">
       ...
       callEvent(obj,outputParams){ 
        ...
        getRefValue(){ 
             ...
             if(true||this.isEditable){ //grid 里面列 全部获取
                _obj  = {};
                let _valueArr = [];
                _valueArr.push({key:this.hiddenValue,value:this.value});
        ...
        goRefWf(item){
            ...
            getWFViewOperateId(_wfId,formView,_ccId).then((response)=>{
                    if(response.data.status <= 99){
                            ...
                            let goPage = "/wh/jsp/version3/flowform/index.html#/wfViewDetail/"+item.hiddenValue+"/"+operateId;
...
<style scoped>
.handleGridRelWFGrid .viewRelWfDiv{
    line-height: 32px;
    margin-bottom:4px;
}

.handleGridRelWFGrid .viewRelWfClass{
    color:#606266;
    font-size: 14px;
    margin-left:10px;
    cursor: pointer;
...
.handleGridRelWFGrid .viewRelWfClass:hover{
    color:cornflowerblue;
}

.handleGridRelWFGrid .iptReadonlyDiv{
