<template>
  <div class="handleGridUserSelectVue designItem designGridItem" >
            <el-input 
                :value="getFilterValue(value)" 
                readonly 
                        v-bind:class="{'iptReadonly':(!isEditable|| isReadonly),'pointerClass':isEditable}"
                        :placeholder="(false && mItem.inst && mItem.inst !='')?mItem.inst:'请选择人员'"
            ...
            if(this.isEditable ){
                ...
                if(this.hiddenValue &&  this.hiddenValue!=''){
                     if(this.mValue.orgChooseOne == 0){ //不允许单选
                        ...
                        if(_length <=1){
                                return {status:1,msg:this.mItem.itemName+' 必须多选(数据方正第'+(this.rowIdx+1)+'行，第'+(this.formIdx+1)+'列)',checkType:'USERSLT',checkId:'handleItem_'+this.mItem.viewId+'_'+ this.mItem.itemId+'_'+this.rowIdx+'_'+this.formIdx} ;
                        }
                    }
                }
            }
