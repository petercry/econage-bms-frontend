<template>
  <div class="handleGridTextareaVue designItem designGridItem">
             <el-input 
                v-model="value" 
                :readonly=" (!isEditable || isReadonly)"  
                v-bind:class="{'iptReadonly':(isReadonly|| isReadonly),'iptPage':hasClick,'pointerCalss':hasClick}" 
                type="textarea" 
...
export default{
  ...
  mounted(){
       ...
       if((this.valHeader.onclickEvents && this.valHeader.onclickEvents.length > 0)){
            this.hasClick = true;
       }
       /*类型*/
       ...
       getRefValue(){ 
        ...
        getRefCheck(){ //检查 是否可以提交
             if(this.isEditable ){
                if(this.isRequired ){
                    if(!this.value || this.value == ''){
                        return { status:1,msg:this.mItem.itemName+' 必须填写(数据方正第'+(this.rowIdx+1)+'行，第'+(this.formIdx+1)+'列)',checkType:'TEXTAREA',checkId:'handleItem_'+this.mItem.viewId+'_'+ this.mItem.itemId+'_'+this.rowIdx+'_'+this.formIdx} ;
                    }
                }
            }
            return {status:0}
