<template>
  <div class="handleInputVue designItem" v-show="isVisible">

      <ecoField :titleWidth="mItem.titleWidth?mItem.titleWidth:defaultTitleWidth" 
        :bgColor="mItem.bgColor?mItem.bgColor:mForm?mForm.titleBgColor:null" 
  ...
  methods: {
       ...
       getRefCheck(){ 
            if(this.isEditable ){
                ...
                if(!EcoUtil.isNumber(this.value,this.mValue.constraintType)){
                        return { status:1,msg:this.mItem.itemName+' 格式错误',checkType:'TEXTFIELD',checkId:'handleItem-'+this.mItem.viewId+'-'+this.mItem.itemId,itemId:this.mItem.itemId} ;
                }
            }
            return {status:0}
        },
