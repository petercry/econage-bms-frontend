<template>
  <div class="handleSealVue designItem" v-show="isVisible" style="z-index:auto">
...
import {getSealById} from'../../../service/service'
...
export default{
  ...
  mounted(){
       ...
       /*向上冒出签章事件*/
        if(this.mValue.sealCode && this.mValue.sealCode!=''){
            this.sealList = this.mValue.sealCode.split(",");
            (this.sealList).forEach((item)=>{
                this.$set(this.sealMap,String(item),'');
                this.getSealSmallSrc(item);
            })
        }
  ...
  methods: {
      ...
      doRefCheck(obj){
        ...
        getSealSmallSrc(sealCode){
            getSealById(sealCode).then((response)=>{
                this.$set(this.sealMap,String(sealCode),'data:image/png;base64,'+response.data.imgBase64);
            })
        },
        ...
        emitMirrorEvent(){ 
            if(this.mValue.itemIdToViewList && this.mValue.itemIdToViewList.length > 0){
                ...
                _emit.data.mirrorData.sealList = EcoUtil.objDeepCopy(this.sealList);
                _emit.data.mirrorData.sealMap = EcoUtil.objDeepCopy(this.sealMap);
        ...
        callMirror(data){
            if(data.mirrorData){
                ...
                this.sealList = data.mirrorData.sealList;
                this.sealMap = data.mirrorData.sealMap;
            }
        }
        /*镜像映射 结束*/
  },
