<template>
  <div class="ecoApprovalSuggestVueStyle2 ecoZJVue" v-show="showflag">
        <div class="ecoApprovalSuggestDiv">
            <el-radio-group v-model="value" size="mini" :disabled="!isEditable"  @change="onChangeEvent" :id="'handleItem'+mItem.itemId">
                <el-radio v-bind:class="{argee:(item.id=='1'),disagree:(item.id=='0'),}" v-show="item.valid !='N' && item.idString" size="mini" :label="String(item.idString)" v-for="item in mValue.KVMap" :key="item.idString">{{item.desc}}</el-radio>
...
.ecoApprovalSuggestVueStyle2 .icon-shanchu1{
    font-size: 13px;
    padding-left:5px;
    padding-right:5px;
}

.ecoApprovalSuggestVueStyle2 .ecoApprovalSuggestDiv{
    line-height: 32px;
    margin:10px 0px;
}
