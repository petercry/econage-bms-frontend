<template>
  <div class="handleCheckboxVue designItem" v-show="isVisible" >
        <ecoField :titleWidth="mItem.titleWidth?mItem.titleWidth:defaultTitleWidth" :bgColor="mItem.bgColor?mItem.bgColor:mForm?mForm.titleBgColor:null" 
            ...
            >
              ...
              <div slot="content" style="line-height: normal;margin: 10px 0px"> 
                    <el-checkbox-group v-model="checkList" @change="onChangeEvent" :disabled="(!isEditable || isReadonly)" :id="'handleItem-'+mItem.viewId+'-'+mItem.itemId">
                        <el-checkbox  
                            v-if="item.enableInCreate || (!isEditable && showItemMap[String(item.id)] )"
            ...
            if(this.isEditable){
                if(this.isRequired ){
                    if(this.checkList.length ==0){
                        return {status:1,msg:this.mItem.itemName+' 必须选择',checkType:'CHECKBOX',checkId:'handleItem-'+this.mItem.viewId+'-'+this.mItem.itemId,itemId:this.mItem.itemId} ;
                    }
                }
            }
            return {status:0}
