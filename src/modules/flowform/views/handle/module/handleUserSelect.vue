<template>
  <div class="handleUserSelectVue designItem" v-show="isVisible">
        ...
        <ecoField :titleWidth="mItem.titleWidth?mItem.titleWidth:defaultTitleWidth" :bgColor="mItem.bgColor?mItem.bgColor:mForm?mForm.titleBgColor:null" 
                :titlePos="mItem.titlePos" :required="isRequired" v-bind:class="{'iptReadonly':(!isEditable || isReadonly)}"
                ...
                <div slot="label"  v-bind:style="{textAlign:mItem.titleAlign}">
                 ...
                 <div slot="content"> 
                         <el-popover
                                    ...
                                    >
                                    <el-row style="background-color:#fafafa"  v-loading="searchLoading">
       ...
       getRefCheck(){ //检查 是否可以提交
            if(this.isEditable ){
                if(this.isRequired ){
                    if(!this.hiddenValue || this.hiddenValue == ''){
                        return {status:1,msg:this.mItem.itemName+' 必须选择',checkType:'USERSLT',checkId:'handleItem-'+this.mItem.viewId+'-'+this.mItem.itemId,itemId:this.mItem.itemId} ;
                    }
                }

                if(this.hiddenValue &&  this.hiddenValue!=''){
                     if(this.mValue.orgChooseOne == 0){ //不允许单选
                        let _length = 0;
                        if((this.mItem.actionGroup || this.mItem.actionGroup == 0 )){
                            _length = ( this.hiddenValue.split('|').length -1 )
                        }else{
                            _length = ( this.hiddenValue.split('|').length)
                        }
                        if(_length <=1){
                            return {status:1,msg:this.mItem.itemName+' 必须多选',checkType:'USERSLT',checkId:'handleItem-'+this.mItem.viewId+'-'+this.mItem.itemId,itemId:this.mItem.itemId} ;
                        }
                    }
                }
            }
        ...
        changeSearchKey(){
            if(this.searchKey && this.searchKey !=''){
                 ...
                 this.searchUserFunc(this.searchKey);
            }else{
        ...
        searchUserFunc(){
                searchUser(this.operateId,this.mItem.itemId,this.searchKey).then((response)=>{
