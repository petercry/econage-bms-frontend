<template>
  <div class="handleDateVue designItem" v-show="isVisible">

       <ecoField :titleWidth="mItem.titleWidth?mItem.titleWidth:defaultTitleWidth" :bgColor="mItem.bgColor?mItem.bgColor:mForm?mForm.titleBgColor:null" :titlePos="mItem.titlePos" 
            v-bind:class="{'iptReadonly':(!isEditable || isReadonly)}" :required="isRequired" 
    ...
    >
            ...
            <div slot="content">
                    <el-time-picker
                        ...
                        :format="mValue.format"
                        :value-format="mValue.format"
...
export default{
  ...
  mounted(){
        ...
        if(this.mValue.format == 'yyyy-MM-dd'){
              this.type = 'date';
        }else if(this.mValue.format == 'yyyy-MM-dd HH:mm'){
              this.type = 'datetime';
        }else if(this.mValue.format == 'yyyy-MM-dd HH'){
              this.type = 'datetime';
        }else if(this.mValue.format == 'yyyy-MM'){
              this.type = 'month';
        }else if(this.mValue.format == 'HH:mm'){
              this.type = 'time';
        }
            ...
            if(this.isEditable){
                if(this.isRequired ){
                    if(!this.value || this.value == ''){
                       return {status:1,msg:this.mItem.itemName+" 必须填写",checkType:'TIMEFIELD', checkId:'handleItem-'+this.mItem.viewId+'-'+this.mItem.itemId,itemId:this.mItem.itemId} ;
                    }
                }
            }
            return {status:0}
