<template>
  <div class="flowFormVue designTable flowDetailVue" >
    ...
    <eco-content top="0px" height="40px" ref="content1"  style="background-color:#fafafa">
          <wfDetailToolbar :formWf="formWf" :formTask="formTask" ref="wfDetailToolbar" ></wfDetailToolbar>
    </eco-content>

    <eco-content top="40px" bottom="0px" ref="content2"  style="background-color:#f4f4f5">
                
                <div class="flowForm" v-bind:style="{width:(formWidth+60)+'px',marginBottom:showApprovalDiv?0:'60px'}">
                    <div style="padding-left:30px;padding-right:30px;" v-bind:class="['formContent'+views.length]">
...
import {getWFFormAjax,getWFFormRender,getWFFormTaskLoad,wfSaveFormAjax,wfSubmitTaskAjax,postWFAjaxEvent,getSeqNumViewAjaxEvent,getPublicSettingUnion} from'../../service/service'
import {sysEnv} from '../../config/env.js'
import {defaultFormWidth,defaultFormTitleTextColor,defaultFormTitleBgColor} from '../../config/setting.js'
import {EcoFile} from '@/components/file/main.js'
import {decimal,baseMainServerUrl} from '../../config/env.js'
...
export default{
  ...
  computed:{
    ...
    showApprovalDiv:function(){
        return this.approvalRowShowFlag !=this.formApprovalItems.length;
    },
    ...
    hasRefKmLinkList(){
        return this.relKlgArray && this.relKlgArray.length>0;
    },
    initActiveLinkName(){
        if(this.hasRefKmLinkList){
            this.activeLinkName = 'handleRefKmLink';
            return 'handleRefKmLink';
        }
    ...
    isHandleOpenDialog:function(){
        if(true || window.flowformSetting && window.flowformSetting.wfDetailHandleType && window.flowformSetting.wfDetailHandleType == 'openDialog'){
            return true;
        }else{
            return false;
        }
            ...
            axios.all([getWFFormRender(this.operateId), getWFFormTaskLoad(this.operateId)])
                    .then(axios.spread(function (res1, res2) {
                        if( res1.data && res1.data.remap.load){
                            ...
                            that.formHandleItems = [{itemId:'EcoHandle'}];
