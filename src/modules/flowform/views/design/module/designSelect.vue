...
<div class="designSelect designItem">
      <ecoField :titleWidth="itemObj.titleWidth"  :bgColor="itemObj.bgColor" :titlePos="itemObj.titlePos" :required="itemObj.nullable == 0" :textAlign="itemObj.titleAlign" 
            ...
            <div slot="content"> 
                     <el-select :value="itemObj.value" placeholder="请选择" size="mini" v-bind:style="{width:itemObj.itemWidth==0?'auto':(itemObj.itemWidth>100?100:itemObj.itemWidth)+'%'}">
                            <el-option
...
<script>
import {defaultTitleWidth,deafultSelectItemWidth}  from'../../../config/setting.js'
...
export default{
  ...
  computed:{
        itemObj(){
            ...
            _item.itemWidth = this.mConfig?this.mConfig.itemWidth:this.mItem.itemWidth;//标题宽度
            ...
            if(_item.itemWidth){
                _item.itemWidth = Number(_item.itemWidth);
            }else{
                _item.itemWidth = deafultSelectItemWidth;
            }
            if(_item.itemWidth <=deafultSelectItemWidth){_item.itemWidth = deafultSelectItemWidth}
