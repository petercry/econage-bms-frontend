<template>
    <div class="designInputSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
        <div class="itemVueName">单行输入框</div>
        <div class="setting">
       ...
       getItemModelCreateApplyFunc(){ 
            getItemModelCreateApplyAjax(this.formOperateId,this.config.modelType,1).then((response)=>{
                if(response.data.status <= 99){
                        ...
                        this.defaultIdArray = [];  /*初始化缺省项*/
                        if(response.data.remap && response.data.remap.default_kv){
                            this.defaultIdArray = response.data.remap.default_kv;
                        }
                      
                        this.initConfig(_itemOperateId,_itemId);  /*初始化配置信息*/
                        this.createItem();
                }else{
