<template>
    <div class="designUserSelectSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
        <div class="itemVueName"><span @click="goBack" class="pointerCalss"><i class="icon iconfont iconback back"></i></span> 明细:选择人员</div>
        <div class="setting">
                ...
                <div style="margin-top:10px;">
                    <el-row class="ecoSettingDesc">
                            <el-col :span="24" class="switchDesc">  
                                <el-switch v-model="config.selectUserByDept" size="mini"></el-switch>
                                <span class="">可选中机构</span>
                            </el-col>

                            <el-col :span="24" class="switchDesc">  
                                <el-switch v-model="config.chooseOne" size="mini"></el-switch>
...
export default{
  name:'designGridUserSelectSettingVue',
  components:{
        tagSelect
  },
  data(){
  ...
  created(){
        this.listAction();
        this.initItem();
        window.ecoDesignGridUserSelectVm = this;
  },
  methods: {
        ...mapMutations([
        ...
        listAction(){
            let callBackDialogFunc = function(obj){
                if(obj && obj.action == 'formulaCallBack'){
                    window.ecoDesignGridUserSelectVm.setFormulaCallBack(obj.data);
                }
            }
            EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'ecoDesignGridUserSelectVmCallBack');
        },
       ...
       getItemModelUpdateApplyFunc(){ 
            getItemModelUpdateApplyAjax(this.formOperateId,this.itemId).then((response)=>{
                if(response.data.status <= 99){
                        ...
                        /*初始化选择限制*/
                        this.subTypeArray = [];
                        if(response.data.remap && response.data.remap.user_subtype){
                            (response.data.remap.user_subtype).forEach((item)=>{
       ...
       initConfig(_itemOperateId,_itemId,remap){
            ...
            _config.modelType = 'USERSLT',
            _config.changeLine = remap?remap.item_model.changeLine:1; //默认换行
            _config.titleName = remap?remap.item_model.titleName:'选择人员';
            ...
            _config.subTypeId = remap?remap.item_model.subTypeId:16; //选择限制 选择人员（单选）
            ...
            _config.asnVal = remap?remap.item_model.asnVal:null;
            if(_config.asnVal){
                try{
                     _config.selectUserByDept =  _config.asnVal[0].selectUserByDept == 1?true:false;
                }catch(e){
                    console.log(e);
                }

                try{
                     _config.chooseOne =  _config.asnVal[0].chooseOne == 1?true:false;
                }catch(e){
                    console.log(e);
                }
            }else{
                _config.selectUserByDept = false;
                _config.chooseOne = true;
            }
  ...
  beforeDestroy(){
         ...
         delete window.ecoDesignGridUserSelectVm;
  }
