 ...
 computed:{
  ...
  methods: {
        ...
        listAction(){
            let callBackDialogFunc = function(obj){
                if(obj && obj.action == 'formulaCallBack'){
                    window.ecoDesignGridInputVm.setFormulaCallBack(obj.data);
                }else if(obj && obj.action == 'formulaSettingV2Callback'){
                    window.ecoDesignGridInputVm.setFormulaCallBackV2(obj.data);
                }
            }
            EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'ecoDesignGridInputVmCallBack');
        },
       ...
       initConfig(_itemOperateId,_itemId,remap){
            ...
            _config.modelType = 'TEXTFIELD',

            _config.changeLine = remap?remap.item_model.changeLine:1; //默认换行
            _config.titleName = remap?remap.item_model.titleName:'单行输入框';
