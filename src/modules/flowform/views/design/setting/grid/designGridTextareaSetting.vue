<template>
    <div class="designTextareaSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
...
import {getItemModelCreateApplyAjax,createFormItemModelAjax,updateFormItemModelAjax,getItemModelUpdateApplyAjax} from'../../../../service/service'
 ...
 computed:{
  ...
  methods: {
        ...
        listAction(){
            let callBackDialogFunc = function(obj){
                if(obj && obj.action == 'formulaCallBack'){
                    window.ecoDesignGridTextareaVm.setFormulaCallBack(obj.data);
                }else if(obj && obj.action == 'formulaSettingV2Callback'){
                    window.ecoDesignGridInputVm.setFormulaCallBackV2(obj.data);
                }
            }
            EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'ecoDesignGridTextareaVmCallBack');
          },
       ...
       initConfig(_itemOperateId,_itemId,remap){
            ...
            _config.modelType = 'TEXTAREA',

            _config.changeLine = remap?remap.item_model.changeLine:1; //默认换行
            _config.titleName = remap?remap.item_model.titleName:'多行输入框';
...
<style scoped>
.designTextareaSettingVue .setting{
    margin:10px 20px 50px 20px;
}

.designTextareaSettingVue .itemVueName{
    Font-weight: bold;
...
.designTextareaSettingVue .optionsDiv{
    font-size: 13px;
    line-height: 30px;
    height:30px;
...
.designTextareaSettingVue .optionsDiv .iconfont{
    vertical-align: middle;
}

.designTextareaSettingVue .optionsDiv .meixuanzhong{
    color:rgba(0, 0, 0, 0.65);
}

.designTextareaSettingVue .optionsDiv .xuanzhong{
    color:#409eff;
}

.designTextareaSettingVue .optionsDiv .handle{
    color:#409eff;
    cursor: move;
    font-size: 16px;
}
.designTextareaSettingVue .optionsDiv .deleteicon{
    color:#409eff;
    cursor: pointer;
    font-size: 16px;
}
.designTextareaSettingVue .optionsDiv input{
    vertical-align: middle;
    border:0px;
    outline:none;
    color:rgba(0, 0, 0, 0.65);
...
.designTextareaSettingVue  .addOptions{
...
.designTextareaSettingVue .el-select{
    width:100%;
}
