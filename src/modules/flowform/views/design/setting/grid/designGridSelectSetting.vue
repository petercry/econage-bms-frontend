 ...
 computed:{
  ...
  methods: {
       ...
       listAction(){
            let callBackDialogFunc = function(obj){
                if(obj && obj.action == 'formulaCallBack'){
                    window.ecoDesignGridSelectVm.setFormulaCallBack(obj.data);
                }
            }
            EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'ecoDesignGridSelectVmCallBack');
        },
       ...
       initConfig(_itemOperateId,_itemId,remap){
            ...
            _config.modelType = 'SLT',
            _config.changeLine = remap?remap.item_model.changeLine:1; //默认换行
            _config.titleName = remap?remap.item_model.titleName:'下拉选框';
