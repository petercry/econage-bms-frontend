<template>
    <div class="designRadioSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
            ...
            <div class="ecoSettingBlock">
                ...
                <div>
                        ...
                        <el-select v-model="config.sysKeyValueOptionsValue" filterable placeholder="请选择" @change="onSysKeyValueOptionsChange">
                            ...
                            <el-select v-model="config.sysOptionsDefautl" clearable  placeholder="请选择">
                                        <el-option
...
import {getItemModelUpdateApplyAjax,getBasicKvCategoryList,getBasicKvGroupList} from'../../../../service/service'
 ...
 computed:{
  ...
  methods: {
        ...
        listAction(){
            let callBackDialogFunc = function(obj){
                if(obj && obj.action == 'formulaCallBack'){
                    window.ecoDesignGridRadioVm.setFormulaCallBack(obj.data);
                }
            }
            EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'ecoDesignGridRadioVmCallBack');
        },
       ...
       initConfig(_itemOperateId,_itemId,remap){
            ...
            _config.modelType = 'RADIO',
            _config.changeLine = remap?remap.item_model.changeLine:1; //默认换行
            _config.titleName = remap?remap.item_model.titleName:'单选框';
            ...
            _config.sysOptions = [];
            _config.sysOptionsDefautl = null;
            ...
            if(!remap){ //没有remap
                ...
                getBasicKvGroupList(_config.sysKeyValueOptionsValue).then((response)=>{ 
                        ...
                        this.config.sysOptionsDefautl = remap.item_model.defaultId;
       ...
       onSysKeyValueOptionsChange(val){ //系统基础数据改变
            ...
            this.config.sysOptionsDefautl = null;
...
<style scoped>
.designRadioSettingVue .setting{
    margin:10px 20px 50px 20px;
}

.designRadioSettingVue .itemVueName{
    Font-weight: bold;
...
.designRadioSettingVue .optionsDiv{
    font-size: 13px;
    line-height: 30px;
    height:30px;
...
.designRadioSettingVue .optionsDiv .iconfont{
    vertical-align: middle;
}

.designRadioSettingVue .optionsDiv .meixuanzhong{
    color:rgba(0, 0, 0, 0.65);
}

.designRadioSettingVue .optionsDiv .xuanzhong{
    color:#409eff;
}

.designRadioSettingVue .optionsDiv .handle{
    color:#409eff;
    cursor: move;
    font-size: 16px;
}
.designRadioSettingVue .optionsDiv .deleteicon{
    color:#409eff;
    cursor: pointer;
    font-size: 16px;
}
.designRadioSettingVue .optionsDiv input{
    vertical-align: middle;
    border:0px;
    outline:none;
    color:rgba(0, 0, 0, 0.65);
...
.designRadioSettingVue  .addOptions{
...
.designRadioSettingVue .el-select{
    width:100%;
}
