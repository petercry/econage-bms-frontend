<template>
    <div class="designInputSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
        <div class="itemVueName"><span @click="goBack" class="pointerCalss"><i class="icon iconfont iconback back"></i></span> 明细:流程关联</div>
        <div class="setting">
...
export default{
  name:'designGridRelWFSettingVue',
  components:{
 ...
 computed:{
  ...
  created(){
      this.listAction();
      this.initItem();
      window.ecoDesignGridRelWFVm = this;
        ...
        listAction(){
            let callBackDialogFunc = function(obj){
                if(obj && obj.action == 'relWFSetting'){
                    window.ecoDesignGridRelWFVm.setRelWFCallBack(obj.data);
                }
            }
            EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'ecoDesignGridRelWFVmCallBack');
        },
        ...
        goRelWFSetting(){
                    let _url = '';
                      if(this.config.wfSceneId == '0'){
                        _url = '/flowform/index.html#/linkWfSetting/'+this.config.itemOperateId+'/0';
                    }else{
                        _url = '/flowform/index.html#/linkWfSetting/'+this.config.itemOperateId+'/'+this.config.wfSceneId;
                    }
                     ...
                     if(sysEnv == 1){
                        ...
                        this.$router.push({name:'linkWfSetting',params:{operateId:this.config.itemOperateId,scId:this.config.wfSceneId == '0'?0:this.config.wfSceneId}});
                    }
