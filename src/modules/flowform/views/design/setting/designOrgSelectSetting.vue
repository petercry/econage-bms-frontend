...
export default{
  ...
  methods: {
       ...
       initConfig(_itemOperateId,_itemId,remap){
            ...
            _config.defaultOrgLevel = remap?remap.item_model.defaultOrgLevel:null; //发起人所在

            _config.selectAsnType = remap?remap.item_model.asnType:0; //选择范围
            _config.selectAsnLg = 0;
            _config.selectAsnVal = null;//默认的内容    
            
            let _asnVal = remap?(remap.item_model.asnVal?remap.item_model.asnVal:[]):[];
            try{
       ...
       changeDefaultId(val){ //选项内容切换的时候变化
           if(val == '41'){
               if(!this.config.defaultOrgLevel || this.config.defaultOrgLevel == 1){
                   if(this.systemOrglevelArray.length > 0){
                         this.config.defaultOrgLevel = this.systemOrglevelArray[0].id;
                   }
               }
           }
       },
      ...
      tagSelectCB(data){
            this.selectAsnOrgValHidden = data.itemStr; 
            this.setSelectAsnVal();
       },
