<template>
    <div class="dirctionOption">
        <div class="btn_line" v-if="data.length==0"><el-button @click="addItem" type="text"><i class="iconfont icon iconicon-test"></i> 添加条件</el-button></div>
        <div class="options" v-if="data.length!=0">
            <div  v-for="(item,index) in data" :key="index">
                <div class="desc_line" v-if="data.length>1 && index>0" ><p>或者</p></div>
                <div class="item">
                    <div style="position: relative;margin:5px 0;" v-for="(parent,index1) in item.logicConds" :key="index1">
                        <div class="itemDesc_line" style="" v-if="item.logicConds.length>1 && index1>0" ><p>并且</p></div>
                        <div class="logicConds" >
                            <el-cascader
                              size="mini"
                              style="width:166px;"
                              v-model="parent.lParent_temp"
                              :options="model_options_temp"
                              @change="handleLeftChange(index,index1,parent)"
                            ...
                            <el-select v-show="parent.showLeftTaskItems" @change="task_change($event,parent)" size="mini" style="width: 95px;" v-model="parent.taskValue">
                                <el-option
                                :key="index"
                                v-for="(item,index) in parent.TASK_ITEMS"
                                :label="item.optionName"
                                :value="index">
                                </el-option>
                            </el-select>
                             <el-select v-show="needAppr(parent) && parent.modelType != 'APPR_CODE,APPR_CODE_RATE'" size="mini" style="width: 95px;" v-model="parent.cSymbol">
                                <el-option
                                :key="index"
                                v-for="(item,index) in parent.formula"
                                :label="item.label"
                                :value="item.value">
                                </el-option>
                             </el-select>
                             <el-select v-show="needAppr(parent)" value-key="idString" @change="appr_change($event,parent)" size="mini" style="width: 95px;" v-model="parent.apprValue">
                                <el-option
                                :key="index"
                                v-for="(item,index) in parent.APPrItemS"
                                :label="item.text"
                                :value="item">
                                </el-option>
                            </el-select>
                            <el-input class="pointerCalss" size="mini" type="text" style="width: 92px;" readonly v-model="parent.lParent" @click.native="selectConnector('3commonModelOption',parent)" v-if="parent.modelType =='DATAMODEL_API' ">

                            </el-input>
                            <el-select v-show="needFormula(parent)" size="mini" style="width: 95px;" v-model="parent.cSymbol">
                                <el-option
                                :key="index"
                                v-for="(item,index) in parent.formula"
                                :label="item.label"
                            ...
                            <div style="display:inline-block" v-if="!parent.changeFormItem">
                                <el-input size="mini" v-if="needInput(parent)" :type="inputType" style="width: 166px;" v-model="parent.rVal"></el-input>
                                <el-date-picker size="mini" v-if="needDate(parent)" style="width: 167px;" type="datetime" value-format="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" v-model="parent.rVal"></el-date-picker>
                                <el-input-number size="mini" v-if="needNumber(parent)" style="width: 166px;" v-model="parent.rVal" controls-position="right" ></el-input-number>
                                <el-input-number size="mini" v-if="needMinNumber(parent)" style="width: 100px;" v-model="parent.rVal" :max="1.0" :min=0 controls-position="right" ></el-input-number>
                                <el-select v-if=" parent.kvName && kvMap.hasOwnProperty(parent.kvName) && kvMap[parent.kvName].length>0" size="mini" v-show="needSelect(parent)" style="width: 166px;" v-model="parent.rVal">
                                    <el-option
                                    :key="index"
                                    v-for="(item,index) in kvMap[parent.kvName]"
                                    :label="item.text"
                                    :value="item.text">
                                    </el-option>
                                </el-select>
                                <el-select  v-if=" parent.kvName  && kvMap.hasOwnProperty(parent.kvName)  && kvMap[parent.kvName].length>0"  size="mini" @change="MultSelect($event,parent)" v-show="needMultSelect(parent)" :multiple="true" style="width: 166px;" v-model="parent.rItem_temp">
                                    <el-option
                                    :key="index"
                                    v-for="(item,index) in kvMap[parent.kvName]"
                                    :label="item.text"
                                ...
                                <tag-select v-if="needSelectPerson(parent)" style="width: 166px;vertical-align: top;"  :initDataStr="parent.rVal"  :initOptions="{selectType:parent.modelType == 'ORGSLT' ? 'Dept':'User'}" @callBack="tagSelectCB($event,parent)" ></tag-select>
                            </div>
                            <el-cascader
                              size="mini"
                              style="width:166px;"
                              v-model="parent.rParent_temp"
                              :options="model_options_temp"
                              :ref="'modelRightOptions'+index+ '|' +index1"
                               @change="handleRightChange(index,index1,parent)"
                             :props="{disabled:'disabled1', label:'optionName',leaf:'1',value:'optionId',children:'deriveItems'}"
                              v-show="parent.changeFormItem"
                            >
                            ...
                            <el-select v-show="parent.showRightTaskItems" @change="task_right_change($event,parent)" size="mini" style="width: 95px;" v-model="parent.rightTaskValue">
                                <el-option
                                :key="index"
                                v-for="(item,index) in parent.R_TASK_ITEMS"
                                :label="item.optionName"
                                :value="index">
                                </el-option>
                            </el-select>
                            <el-input class="pointerCalss" size="mini" type="text" style="width: 92px;" readonly v-model="parent.rParent" @click.native="selectConnector('4commonModelOption',parent)" v-if="parent.showRightApiInput">

                            </el-input>
                            <div class="qiehuan" @click="changeFormItem(parent)" v-show="needFormula(parent) && !needAppr(parent)">
...
export default{
  ...
  created(){
     window.commonModelOptionVM = this;
  },
  mounted(){
     this.model_options_temp = this.copyDeep(this.modelOption);
     this.init();
     this.bindAction();
  ...
  methods: {
       bindAction(){
        ...
        let callBackDialogFunc = function(obj){
            console.log('commonModelOption 监控回写');
            if(obj && obj.action == 'selectConnector' ){

                if(obj.flag == '3commonModelOption'){//左边选择
                    window.commonModelOptionVM.needIngItem.lParent = obj.data.paramName;
                    window.commonModelOptionVM.needIngItem.lItem = obj.data.paramId?obj.data.paramId:obj.data.paramDataId?obj.data.paramDataId:"";
                }else if(obj.flag == '4commonModelOption'){//右边选择
                    window.commonModelOptionVM.needIngItem.rParent = obj.data.paramName;
                    window.commonModelOptionVM.needIngItem.rItem = obj.data.paramId?obj.data.paramId:obj.data.paramDataId?obj.data.paramDataId:"";
                }
            }
            return ;
        }
        EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'commonModelOption');
        console.log('common');
        console.log(EcoUtil.callBackFuncObj);
      },
      modelSelect(index,item){
        item.lMd = this.modelOption[index].model.mdId;
      ...
      itemsSelect(index,item){
        item.lItem = this.modelOption[item.index].items[index].dataId;
        item.formItem = this.modelOption[item.index].items[index].kvName;
        item.modelType = this.modelOption[item.index].items[index].modelType;
        item.rMd = 0;
        this.$set(item,'rVal',''); //添加属性或修改属性
        this.$set(item,'rItem',''); //添加属性或修改属性
        this.$set(item,'rItem_temp',''); //添加属性或修改属性
            ...
            element.logicConds.forEach((item,index1) => {
                ...
                if(item.rParent && !item.rVal){
                    ...
                    if(item.rItem && item.rItem != item.rParent_temp[item.rParent_temp.length - 1]){
                        ...
                        let r_node = this.getCascaderObj(item.rParent_temp,this.modelOption);
                ...
                let node = this.getCascaderObj(item.lParent_temp,this.modelOption);
                ...
                this.$set(item,'APPrItemS',new Array);
                 if(item.lItem != item.lParent_temp[item.lParent_temp.length-1] && data.modelType != "DATAMODEL_API"){
                   ...
                   if(item.modelType == "APPR_CODE"){
                      this.APPrItemS = this.kvMap[data.kvName];
                      let obj = {
      ...
      handleLeftChange(index,index1,item) {
          ...
          if(item.lParent_temp.length > 1){
              ...
              if(node.hasOwnProperty("taskItems") && item.modelType == "TASK_LEVEL"){
                    ...
                    if(item.modelType == "APPR_CODE"){
                        this.APPrItemS = this.kvMap[node.kvName];
           ...
           if(item.lParent_temp.length == 1){
              ...
              if(item.modelType == 'DATAMODEL_API'){
                 ...
                 this.selectConnector('3commonModelOption',item);
              }
           ...
           if(item.rParent_temp.length > 1){
            ...
            if(item.rParent_temp.length == 1){
              ...
              if(node.modelType == "DATAMODEL_API"){
                   ...
                   this.selectConnector('4commonModelOption',item);
      ...
      selectConnector(flag,item){
         window.commonModelOptionVM.needIngItem = item;
...
.dirctionOption .tagSelect .iconfont{
    top:4px !important;
}
</style>
