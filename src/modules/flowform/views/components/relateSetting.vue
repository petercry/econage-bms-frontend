<template>
    <div class="relateSetting">
       <div v-loading="loading">
          <div class="relateItem" v-for="(relateItem,index) in relateItems" :key="index">
              <p style="line-height: 40px;color: #262626;">数据规则 {{index+1}}  <i @click="deleteRelateItem(index)" class="iconfont icon iconclosecircleo" style="color:#1ba5fa;"></i>
              </p>
              <div class="relateOption">
                <div>
                    <p style="line-height: 40px;"><span>当符合以下条件时</span></p>
                    <op-check :data="relateItem.logicSet" :modelOption="modelOption" :kvMap="kvMap" :operateId="flowOperateId"></op-check>
                </div>
              </div>
              <div>

                <div class="relateOption" style="border-top:none;">
                      <p style="line-height: 40px;color: #262626;">则触发以下操作</p>
                    <div>
                        <p class="header"><span style="width:30%;">目标元素</span><span style="width:16%;">改变属性</span><span style="width:52%;">属性值</span></p>
                    </div>
                    <div class="fromItem" v-for="(fromItem,index1) in relateItem.targetList" :key="index1">
                        <div>
                            <div class="con" style="width:30%;display:inline-block;text-align:center">
                              ...
                              <el-cascader
                                ...
                                @change="handleLeftChange(index,index1,fromItem)"
                                :ref="'modelLeftOptions'+index+ '|' +index1"
                            ...
                            <div class="con" style="width:52%;display:inline-block;text-align:left">
                                ...
                                :options="modelOption"
                                @change="handleRightChange(index,index1,fromItem)"
                                ...
                                <div class="qiehuan" @click="deleteFormItem(index1,relateItem)" style="color:rgb(245, 108, 108)">
          ...
          <div class="opRelate">
              <div class="btn-line" @click="addRelateItem(relateItems)">
                  <el-button size="medium"  type="text"><i class="iconfont icon iconicon-test"></i> 添加数据规则</el-button>
              </div>
          </div>
          <div class="btn">
...
import {getRenderInteractionModel,saveInteractionModel} from '../../service/service.js'
...
export default{
  ...
  mounted(){
      this.getRenderInteractionModel();
      this.bindAction();
  },
  computed:{
  ...
  methods: {
      bindAction(){
        ...
        EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'relateSetting');
        console.log('relate');
      ...
      getRenderInteractionModel(){
        this.loading = true;
        getRenderInteractionModel(this.flowOperateId).then((response) => {
              this.loading = false;
                ...
                item.targetList.forEach((single,index2) => {
                    try{
                        ...
                        this.$nextTick(() => { // 以服务的方式调用的 Loading 需要异步关闭
                                let model = 'modelLeftOptions'+index1+ '|' +index2;
      ...
      handleLeftChange(index,index1,item){
          ...
          let model = 'modelLeftOptions'+index+ '|' +index1;
     ...
     changeFormItem(item){
      ...
      onSubmit(){
          ...
          let that = this;
          saveInteractionModel(data).then((response) => {
               this.$nextTick(() => { // 以服务的方式调用的 Loading 需要异步关闭
...
.relateSetting .relateItem {
  padding: 10px 24px 20px 24px;
   border-bottom: 1px solid #e8e8e8;
}
.relateSetting .relateItem .relateOption{
    padding: 16px;
    border: 1px solid #e8e8e8;
    background-color: #fafafa;
}
.relateSetting .relateItem .icon{
  cursor: pointer;
  position: relative;
  top: 1px;
}
.relateSetting .opRelate .btn-line {
