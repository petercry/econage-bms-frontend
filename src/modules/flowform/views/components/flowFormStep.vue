                ...
                <el-col :span="12" style="text-align:-webkit-center;">
                    <div class="ecoStep">
                         <el-steps :active="step" simple size="mini" >
                                ...
                                <el-step title="表单设计"  @click.native="goFlowFrom" v-if="!isCustomer">
                                    <i slot="icon" class="icon iconfont iconnumber1 iconstep pointerCalss"></i>
                                </el-step>

                                <el-step title="表单设计"  @click.native="goFlowFromView" v-else>
                                    <i slot="icon" class="icon iconfont iconnumber1 iconstep pointerCalss"></i>
                                </el-step>

                                <el-step title="流向设计"  @click.native="goDirection">
                                    <i slot="icon" class="icon iconfont iconnumber2 iconstep pointerCalss"></i>
                                </el-step>
                                <el-step title="扩展配置" @click.native="goFlowSetting" v-if="!isCustomer">
                                    <i slot="icon" class="icon iconfont iconnumber3 iconstep pointerCalss"></i>
                                </el-step>
                                <el-step title="扩展配置" class="notallowed" v-if="isCustomer">
                                    <i slot="icon" class="icon iconfont iconnumber3 iconstep pointerCalss"></i>
                                </el-step>
                                <!-- <el-step title="模拟测试"  @click.native="goFlowTest">
                                      <i slot="icon" class="icon iconfont iconnumber4 iconstep pointerCalss"></i>
                                </el-step> -->
                                <el-step title="流程发布"  @click.native="goFlowPublish">
                                     <i slot="icon" class="icon iconfont iconnumber4 iconstep pointerCalss"></i>
                                </el-step>
                         </el-steps>
...
export default{
  ...
  methods: {
        ...
        goFlowFromView(){
            ...
            if(this.checkSave) this.checkSave();
            this.$router.push({name:'formDesignView',params:{
                        formId:this.$route.params.formId,
                        templateId:this.$route.params.templateId,
            }});
        },
