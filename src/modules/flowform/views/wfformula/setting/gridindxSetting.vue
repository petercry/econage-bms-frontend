<template>
    <div class="gridindxParams" v-show="paramsObj.loaded1 && paramsObj.loaded2">
            <div class="itemVueName">GRIDINDX 参数设置</div>
            <div class="setting">
            <div class="ecoSettingBlock" v-for="(paramItem,idx) in paramsObj.paramsArray" :key="idx">
                ...
                <div>
                    <el-radio-group style="margin-bottom: 15px;" v-model="paramItem.type" @change="changeType(idx)">
                        <el-radio-button  label="1" v-if="idx > 0">自定义</el-radio-button>
                ...
                <div v-if="paramItem.type == 2">
                    <el-select  placeholder="请选择" v-model="paramItem.value" @change="changeSelect(idx)">
                            <el-option
                                v-if="(item.modelType == 'GRID' && idx == 0)||(item.modelType == 'NUMFIELD' && idx > 0) "
...
export default{
    ...
    created(){
        ...
        this.funcList.push({name:'CALCULATE',value:'CALCULATE',idx:1});
        this.funcList.push({name:'SUM',value:'SUM',idx:1});
        this.funcList.push({name:'DAYS',value:'DAYS',idx:1});
        this.funcList.push({name:'HOURS',value:'HOURS',idx:1});
        this.funcList.push({name:'YEARS',value:'YEARS',idx:1});
        this.funcList.push({name:'TONUMBER',value:'TONUMBER',idx:1});
        this.funcList.push({name:'COUNT',value:'COUNT',idx:1});

        this.funcList.push({name:'CALCULATE',value:'CALCULATE',idx:2});
        this.funcList.push({name:'SUM',value:'SUM',idx:2});
        this.funcList.push({name:'DAYS',value:'DAYS',idx:2});
        this.funcList.push({name:'HOURS',value:'HOURS',idx:2});
        this.funcList.push({name:'YEARS',value:'YEARS',idx:2});
        this.funcList.push({name:'TONUMBER',value:'TONUMBER',idx:2});
        this.funcList.push({name:'COUNT',value:'COUNT',idx:2});
    ...
    methods: {
        ...
        init(){
            ...
            this.$set(this.paramsObj,'paramsArray',(_paramsObj && _paramsObj.paramsArray)? _paramsObj.paramsArray:[{name:null,value:null,type:2},{name:null,value:null,type:1},{name:null,value:null,type:1}]);
...
.gridindxParams .setting{
    margin:10px 10px 50px 20px;
}

.gridindxParams .itemVueName{
    Font-weight: bold;
...
.gridindxParams .ecoSettingBlock{
    margin-bottom:10px;
}

.gridindxParams .ecoSettingDesc .title{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
...
.gridindxParams .ecoSettingDesc .del{
    font-size: 12px;
    color: #f56c6c;
    height: 32px;
    line-height: 32px;
...
.gridindxParams .ecoSettingDesc .note{
    font-size: 14px;
    height: 32px;
    line-height: 32px;
    color: #8b8b8b;
...
.gridindxParams .funcItem:hover{
    background-color:rgb(233,250,255);
}

.gridindxParams .plainBtn{
    border-color: #409eff;
    color: #409eff;
    font-size: 14px;
    margin-right:10px;
