<template>
    ...
    <div class="minParams" v-show="paramsObj.loaded1 && paramsObj.loaded2">
            <div class="itemVueName">MIN 参数设置</div>
            <div class="setting">
            <div class="ecoSettingBlock" v-for="(paramItem,idx) in paramsObj.paramsArray" :key="idx">
                ...
                <div v-if="paramItem.type == 2">
                    <el-select  placeholder="请选择" v-model="paramItem.value" @change="changeSelect(idx)">
                            <el-option
                                v-if="item.modelType == 'TIMEFIELD' || item.modelType == 'NUMFIELD'"
...
.minParams .ecoSettingBlock{
    margin-bottom:10px;
}

.minParams .ecoSettingDesc .title{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
