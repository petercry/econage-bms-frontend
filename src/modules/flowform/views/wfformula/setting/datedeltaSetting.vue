<template>
    ...
    <div class="datedeltaParams" v-show="paramsObj.loaded1 && paramsObj.loaded2">
            <div class="itemVueName">DATEDELTA 参数设置</div>
            <div class="setting">
            <div class="ecoSettingBlock" v-for="(paramItem,idx) in paramsObj.paramsArray" :key="idx">
                ...
                <div v-if="paramItem.type == 2">
                    <el-select  placeholder="请选择" v-model="paramItem.value" @change="changeSelect(idx)">
                            <el-option
                                v-if="(item.modelType == 'TIMEFIELD' && idx == 0)||(item.modelType == 'NUMFIELD' && idx == 1) "
...
.datedeltaParams .setting{
    margin:10px 10px 50px 20px;
}

.datedeltaParams .itemVueName{
    Font-weight: bold;
...
.datedeltaParams .ecoSettingBlock{
    margin-bottom:10px;
}

.datedeltaParams .ecoSettingDesc .title{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
...
.datedeltaParams .ecoSettingDesc .del{
    font-size: 12px;
    color: #f56c6c;
    height: 32px;
    line-height: 32px;
...
.datedeltaParams .ecoSettingDesc .note{
    font-size: 14px;
    height: 32px;
    line-height: 32px;
    color: #8b8b8b;
...
.datedeltaParams .funcItem:hover{
    background-color:rgb(233,250,255);
}

.datedeltaParams .plainBtn{
    border-color: #409eff;
    color: #409eff;
    font-size: 14px;
    margin-right:10px;
