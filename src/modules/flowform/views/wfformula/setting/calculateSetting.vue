<template>
    ...
    <div class="calculateParams" v-show="paramsObj.loaded1 && paramsObj.loaded2">
            <div class="itemVueName">CALCULATE 参数设置</div>
            <div class="setting">
            <div class="ecoSettingBlock" v-for="(paramItem,idx) in paramsObj.paramsArray" :key="idx">

                <el-row class="ecoSettingDesc">
                        <el-col :span="20" class="title"><span>{{'参数'+(idx+1)}}</span> </el-col>
    ...
    methods: {
        ...
        onInputKeyPress(event){
            if(event.keyCode < 45 || event.keyCode > 57){
                if(event.keyCode == 61 || event.keyCode == 43 || event.keyCode == 42 
                    || event.keyCode == 47 || event.keyCode == 40 || event.keyCode == 41){
...
.calculateParams .setting{
    margin:10px 10px 50px 20px;
}

.calculateParams .itemVueName{
    Font-weight: bold;
...
.calculateParams .ecoSettingBlock{
    margin-bottom:10px;
}

.calculateParams .ecoSettingDesc .title{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
...
.calculateParams .ecoSettingDesc .del{
    font-size: 12px;
    color: #f56c6c;
    height: 32px;
    line-height: 32px;
...
.calculateParams .ecoSettingDesc .note{
    font-size: 14px;
    height: 32px;
    line-height: 32px;
    color: #8b8b8b;
...
.calculateParams .funcItem:hover{
    background-color:rgb(233,250,255);
}

.calculateParams .plainBtn{
    border-color: #409eff;
    color: #409eff;
    font-size: 14px;
    margin-right:10px;
