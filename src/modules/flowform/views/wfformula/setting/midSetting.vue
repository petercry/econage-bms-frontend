<template>
    ...
    <div class="midParams" v-show="paramsObj.loaded1 && paramsObj.loaded2">
            <div class="itemVueName">MID 参数设置</div>
            <div class="setting">
            <div class="ecoSettingBlock" v-for="(paramItem,idx) in paramsObj.paramsArray" :key="idx">
                ...
                <div v-if="paramItem.type == 2">
                    <el-select  placeholder="请选择" v-model="paramItem.value" @change="changeSelect(idx)">
                            <el-option
                                v-if="(idx == 0)||(item.modelType == 'NUMFIELD' && idx >=1) "
...
export default{
    ...
    methods: {
        ...
        init(){
            ...
            this.$set(this.paramsObj,'paramsArray',(_paramsObj && _paramsObj.paramsArray)? _paramsObj.paramsArray:[{name:null,value:null,type:1},{name:null,value:null,type:1},{name:null,value:null,type:1}]);
...
.midParams .ecoSettingBlock{
    margin-bottom:10px;
}

.midParams .ecoSettingDesc .title{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
