<template>
    <span class="gridindxFunc formulaFuncItem" v-bind:class="{'clickActive':isClickActive}"  >
        <span class="desc1" @click="clickParams">GRIDINDX</span>
        ...
        <template v-else>
            ...
            <span  v-bind:class="[paramItem.value?'hasSetDesc':'needSetDesc'] ">{{(paramItem.name) ? paramItem.name: ('参数'+(idx+1))}}</span>
        </template>
...
export default{
    ...
    computed:{
        ...
        paramsObj(){
            ...
            let _item = {};
            _item.paramsArray = _params?_params.paramsArray:[{name:null,value:null,type:2,uuid:null},{name:null,value:null,type:1,uuid:null},{name:null,value:null,type:1,uuid:null}];
    ...
    methods: {
        ...
        clickParams(){
            ...
            _data.paramsName = 'formulaGridindxSetting';
...
.gridindxFunc .needSetDesc i{
    background-color: none;
}

.gridindxFunc .hasSetDesc{
    font-size: 14px;
    padding-left:5px;
    padding-right:5px;
    color:#999;
