<template>
    <span class="concatenateFunc formulaFuncItem" v-bind:class="{'clickActive':isClickActive}"  >

        <span class="desc1" @click="clickParams">CONCATENATE</span>
        ...
        <template v-else>
            ...
            <span  v-bind:class="[paramItem.value?'hasSetDesc':'needSetDesc'] ">{{(paramItem.name) ? paramItem.name: (idx==0?'分隔符':'参数'+(idx))}}</span>
        </template>
...
export default{
    ...
    computed:{
        ...
        paramsObj(){
            ...
            let _item = {};
            _item.paramsArray = _params?_params.paramsArray:[{name:null,value:null,type:1,uuid:null},{name:null,value:null,type:1,uuid:null}];
    ...
    methods: {
        ...
        clickParams(){
            ...
            _data.paramsName = 'formulaConcatenateSetting';
...
.concatenateFunc .desc1{
    color:#fa8e1b;
    font-size: 18px;
    cursor: pointer;
}

.concatenateFunc .desc2{
    font-size: 18px;
    position: relative;
}

.concatenateFunc .needSetDesc{
...
.concatenateFunc .needSetDesc i{
    background-color: none;
}

.concatenateFunc .hasSetDesc{
    font-size: 14px;
    padding-left:5px;
    padding-right:5px;
    color:#999;
