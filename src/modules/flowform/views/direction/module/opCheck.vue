                ...
                <div class="item">
                    <div style="position: relative;margin:5px 0;" v-for="(item1,index1) in item.logicConds" :key="index1">
                        ...
                        <div class="logicConds" >
                            ...
                            <el-select v-show="item1.showLeftTaskItems" @change="task_change($event,item1)" size="mini" style="width: 95px;" v-model="item1.taskValue">
                                <el-option
                                :key="index"
                            ...
                            <el-select v-show="needAppr(item1) && item1.modelType != 'APPR_CODE,APPR_CODE_RATE'" size="mini" style="width: 95px;" v-model="item1.cSymbol">
                                <el-option
                                :key="index"
                             ...
                             <el-select v-show="needAppr(item1)" value-key="idString" @change="appr_change($event,item1)" size="mini" style="width: 95px;" v-model="item1.apprValue">
                                <el-option
                                :key="index"
                            ...
                            <el-input class="pointerCalss" size="mini" type="text" style="width: 100px;" readonly v-model="item1.lParent" @click.native="needIngItem=item1;selectConnector('3opCheck')" v-if="item1.modelType =='DATAMODEL_API' ">

                            </el-input>
                            <el-select v-show="needFormula(item1)" size="mini" style="width: 95px;" v-model="item1.cSymbol">
                                <el-option
                                :key="index"
                                v-for="(item,index) in item1.formula"
                                :label="item.label"
                            ...
                            <div style="display:inline-block" v-if="!item1.changeFormItem">
                                <el-input size="mini" v-if="needInput(item1)" :type="inputType" style="width: 166px;" v-model="item1.rVal"></el-input>
                                <el-date-picker size="mini" v-if="needDate(item1)" style="width: 167px;" type="datetime" value-format="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" v-model="item1.rVal"></el-date-picker>
                                <el-input-number size="mini" v-if="needNumber(item1)" style="width: 166px;" v-model="item1.rVal" controls-position="right" ></el-input-number>
                                ...
                                <el-select v-if=" item1.kvName && directionData.kvMap.hasOwnProperty(item1.kvName) && directionData.kvMap[item1.kvName].length>0" size="mini" v-show="needSelect(item1)" style="width: 166px;" v-model="item1.rVal">
                                    <el-option
                                    :key="index"
                                ...
                                <el-select  v-if=" item1.kvName  && directionData.kvMap.hasOwnProperty(item1.kvName)  && directionData.kvMap[item1.kvName].length>0"  size="mini" @change="MultSelect($event,item1)" v-show="needMultSelect(item1)" :multiple="true" style="width: 166px;" v-model="item1.rItem_temp">
                                    <el-option
                                    :key="index"
                                    v-for="(item,index) in directionData.kvMap[item1.kvName]"
                                    :label="item.text"
                                ...
                                <tag-select v-if="needSelectPerson(item1)" style="width: 166px;vertical-align: top;"  :initDataStr="item1.rVal" :initOptions="{selectType:item1.modelType == 'ORGSLT' ? 'Dept':'User'}" @callBack="tagSelectCB($event,item1)" ></tag-select>
                            </div>
                            <el-cascader
                              size="mini"
                              style="width:166px;"
                            ...
                            <el-select v-show="item1.showRightTaskItems" @change="task_right_change($event,item1)" size="mini" style="width: 95px;" v-model="item1.rightTaskValue">
                                <el-option
                                :key="index"
                            ...
                            <el-input class="pointerCalss" size="mini" type="text" style="width: 89px;" readonly clearable v-model="item1.rParent" @click.native="needIngItem=item1;selectConnector('4opCheck')" v-if="item1.showRightApiInput">
  ...
  methods: {
      bindAction(){
        ...
        let callBackDialogFunc = function(obj){
            console.log('opcheck 监控回写');
            if(obj && obj.action == 'selectConnector' ){

                if(obj.flag == '3opCheck'){//左边选择
                    this_.needIngItem.lParent = obj.data.paramName;
                    this_.needIngItem.lItem = obj.data.paramId?obj.data.paramId:obj.data.paramDataId?obj.data.paramDataId:"";
                }else if(obj.flag == '4opCheck'){//右边选择
                    this_.needIngItem.rParent = obj.data.paramName;
                    this_.needIngItem.rItem = obj.data.paramId?obj.data.paramId:obj.data.paramDataId?obj.data.paramDataId:"";
                }
            }
        }
        EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'opcheck');
      },
          ...
          this.data.forEach((element,index) => {
            element.logicConds.forEach((item,index1) => {
                ...
                this.$set(item,'APPrItemS',new Array);
                 if(item.lItem != item.lParent_temp[item.lParent_temp.length-1] && item.lMd != "3"){
