<template>
    <div class="designGridCheckboxSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
        
        <div class="itemVueName"><span @click="goBack" class="pointerCalss"><i class="icon iconfont iconback back"></i></span> 明细:多选框</div>
         <div class="setting">
                ...
                <el-tab-pane label="视图" name="view">
                        ...
                        <div class="ecoSettingBlock">
                               ...
                               <div>
                                        ...
                                        <el-select v-model="sysOptionsDefautlArr" clearable multiple placeholder="请选择" @change="onSysOptionsDefaultChange">
                                                    <el-option
...
export default{
  name:'designGridCheckboxSettingVue',
  components:{
  ...
  created(){
      ...
      this.initItem();
      window.ecoDesignGridCheckboxVm = this;
  ...
  methods: {
       ...
       initConfig(_fieldId,remap){
            this.fieldTypeArray = [];
            this.fieldTypeArray.push({id:'MULTI_LINK',desc:'MULTI_LINK'});
            ...
            _config.view.display =  remap?remap.view.display:'多选框';
            ...
            _config.view.attrs.optionGrid = _attrsObj?(Number(_attrsObj.optionGrid)):0;
            ...
            if(_config.view.attrs.sysOptionsDefautl){
                 this.sysOptionsDefautlArr = _config.view.attrs.sysOptionsDefautl.split(",");
            }else{
            ...
            _config.model.display = remap?remap.model.display:'多选框',
            _config.model.field = _fieldId ;
            _config.model.fieldType =  remap?remap.model.fieldType:'MULTI_LINK';
      ...
      onSysKeyValueOptionsChange(val,type){ //系统基础数据改变
        ...
        onSysOptionsDefaultChange(val){ //系统基础数据改变
            if(val.length > 0){
                 this.config.view.attrs.sysOptionsDefautl = val.join(",")
            }else{
                 this.config.view.attrs.sysOptionsDefautl = null;
            }
       },
  ...
  beforeDestroy(){
         console.log('销毁组件前调用');
         delete window.ecoDesignGridCheckboxVm;
  }
...
<style scoped>
.designGridCheckboxSettingVue .setting{
    margin:10px 20px 50px 20px;
}

.designGridCheckboxSettingVue .itemVueName{
    Font-weight: bold;
...
.designGridCheckboxSettingVue .optionsDiv{
    font-size: 13px;
    line-height: 30px;
    height:30px;
...
.designGridCheckboxSettingVue .optionsDiv .iconfont{
    vertical-align: middle;
}

.designGridCheckboxSettingVue .optionsDiv .meixuanzhong{
    color:rgba(0, 0, 0, 0.65);
}

.designGridCheckboxSettingVue .optionsDiv .xuanzhong{
    color:#409eff;
}

.designGridCheckboxSettingVue .optionsDiv .handle{
    color:#409eff;
    cursor: move;
    font-size: 16px;
}
.designGridCheckboxSettingVue .optionsDiv .deleteicon{
    color:#409eff;
    cursor: pointer;
    font-size: 16px;
}
.designGridCheckboxSettingVue .optionsDiv input{
    vertical-align: middle;
    border:0px;
    outline:none;
    color:rgba(0, 0, 0, 0.65);
...
.designGridCheckboxSettingVue  .addOptions{
...
.designGridCheckboxSettingVue .el-select{
    width:100%;
}

.designGridCheckboxSettingVue .ecoSettingDesc .title{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
...
.designGridCheckboxSettingVue .ecoSettingDesc .note{
    font-size: 14px;
    height: 32px;
    line-height: 32px;
    color: #8b8b8b;
...
.designGridCheckboxSettingVue  .tip{
    font-size: 12px;
    line-height: 16px;
    color: #8b8b8b;
    font-weight: normal;
...
.designGridCheckboxSettingVue .ecoSettingDesc .switchDesc{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
...
.designGridCheckboxSettingVue .ecoSettingBlock{
    margin-bottom:10px;
}
