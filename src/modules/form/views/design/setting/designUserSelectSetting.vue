<template>
    <div class="designUserSelectSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
        <div class="itemVueName">选择人员</div>
        <div class="setting">
                ...
                <el-tab-pane label="视图" name="view">
                    <div>
                        ...
                        <div class="ecoSettingBlock">
                            ...
                            <div>
                                <el-select v-model="config.view.attrs.subTypeId" placeholder="请选择" @change="changeSubType">
                                    <el-option
                            ...
                            <div style="margin-top:10px;">
                                <el-row class="ecoSettingDesc">
                                        <el-col :span="24" class="switchDesc">  
                                            <el-switch v-model="config.view.attrs.selectUserByDept" size="mini"></el-switch>
                                            <span class="">可选中机构</span>
                                        </el-col>

                                        <el-col :span="24" class="switchDesc">  
                                            <el-switch v-model="config.view.attrs.chooseOne" size="mini"></el-switch>
                                            <span class="">允许单选</span>
                                        </el-col>
                                </el-row>
                            </div>
                        </div>
                        ...
                        <div class="ecoSettingBlock">
                                ...
                                <div>
                                    <el-select v-model="config.view.attrs.defaultId" style="margin-bottom: 15px;" @change="changeDefaultId">
                                            <el-option
                                    ...
                                    <div v-show="config.view.attrs.defaultId == 17">
                                            <el-input 
                                                :value="config.view.attrs.defaultVal" 
                                                readonly 
                                            ...
                                            <input type="hidden" v-model="config.view.attrs.defaultValHidden" :id="'ecoOrg_'+this.uuid"/>
                                            <input type="hidden" v-model="config.view.attrs.defaultVal" :id="'ecoOrg_'+this.uuid+'_text'"/>

                                    </div>
                                    <div v-show="config.view.attrs.defaultId == 40"></div>
                                </div>
                        </div>
...
export default{
  name:'designUserSelectSettingVue',
  components:{
 ...
 computed:{
  ...
  methods: {
       ...
       initConfig(_fieldId,remap){
            ...
            this.fieldTypeArray = [];
            this.fieldTypeArray.push({id:'ORGANIZATION',desc:'ORGANIZATION'});

            /*初始化缺省项*/
            this.defaultIdArray = [];
            this.defaultIdArray.push({value:17,label:'自定义'});
            this.defaultIdArray.push({value:40,label:'发起人'});
            ...
            _config.view.display =  remap?remap.view.display:'选择人员';
            ...
            _config.view.attrs.selectUserByDept = _attrsObj?(String(_attrsObj.selectUserByDept) =='true'?true:false):false;
            _config.view.attrs.chooseOne = _attrsObj?(String(_attrsObj.chooseOne) =='true'?true:false):true;
            ...
            _config.model.display = remap?remap.model.display:'选择人员',
...
<style scoped>
.designUserSelectSettingVue .setting{
    margin:10px 20px 50px 20px;
}

.designUserSelectSettingVue .itemVueName{
    Font-weight: bold;
...
.designUserSelectSettingVue .optionsDiv{
    font-size: 13px;
    line-height: 30px;
    height:30px;
...
.designUserSelectSettingVue .optionsDiv .iconfont{
    vertical-align: middle;
}

.designUserSelectSettingVue .optionsDiv .meixuanzhong{
    color:rgba(0, 0, 0, 0.65);
}

.designUserSelectSettingVue .optionsDiv .xuanzhong{
    color:#409eff;
}

.designUserSelectSettingVue .optionsDiv .handle{
    color:#409eff;
    cursor: move;
    font-size: 16px;
}
.designUserSelectSettingVue .optionsDiv .deleteicon{
    color:#409eff;
    cursor: pointer;
    font-size: 16px;
}
.designUserSelectSettingVue .optionsDiv input{
    vertical-align: middle;
    border:0px;
    outline:none;
    color:rgba(0, 0, 0, 0.65);
...
.designUserSelectSettingVue  .addOptions{
...
.designUserSelectSettingVue .el-select{
    width:100%;
}

.designUserSelectSettingVue .ecoSettingDesc .title{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
...
.designUserSelectSettingVue .ecoSettingDesc .note{
    font-size: 14px;
    height: 32px;
    line-height: 32px;
    color: #8b8b8b;
...
.designUserSelectSettingVue .tip{
    font-size: 12px;
    line-height: 16px;
    color: #8b8b8b;
    font-weight: normal;
...
.designUserSelectSettingVue .ecoSettingDesc .switchDesc{
    font-size: 14px;
    color: #606266;
    height: 32px;
    line-height: 32px;
...
.designUserSelectSettingVue .ecoSettingBlock{
    margin-bottom:10px;
}
