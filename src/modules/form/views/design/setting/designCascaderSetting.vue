...
import {getItemModelCreateApplyAjax,createFormItemModelAjax,getItemModelUpdateApplyAjax,getCascaderDataMapTypeList,getCascaderNodeList,getCascaderNodeLevelNum} from'@/flowform/service/service'
  ...
  methods: {
       ...
       initConfig(_itemOperateId,_itemId,remap){
            ...
            if(!remap){ //没有remap
                ...
                if(_config.csDataSet && _config.csDataSet!=""){
                        let _data = {};
                        _data.parent_id = _config.csDataSet;
                        _data.level_id = 1;
                        _data.ds_id = _config.csDataSet;
                        getCascaderNodeList(_data).then((response)=>{ 
                             this.config.csNodeOptions = response.data.remap.list;
       ...
       createItem(){
        ...
        getDataMapTypeListFunc(){ 
            ...
            getCascaderDataMapTypeList().then((response)=>{
                if(response.data.status<=99){
                     this.dataMapTypeList = response.data.remap.list;
                }
            })
        },
        ...
        getCascaderNodeLevelNumFunc(id,csLevelNumChange){
                getCascaderNodeLevelNum(id).then((response)=>{
                        let _maxLevel = response.data.remap.max_level;
                        if(_maxLevel < 1){
                            _maxLevel = 1;
                        }
