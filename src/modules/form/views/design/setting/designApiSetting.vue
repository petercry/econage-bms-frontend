<template>
    <div class="designApiSettingVue designSettingVue" v-show="config.loaded1 && config.loaded2">
        
        <div class="itemVueName">API关联</div>
        <div class="setting">
...
export default{
 ...
 computed:{
  ...
  methods: {
         ...
         listAction(){
            let callBackDialogFunc = function(obj){
                if(obj && obj.action == 'sceneSetting'){
                    window.ecoDesignSettingVm.setSceneCallBack(obj.data);
                }else if(obj && obj.action == 'viewApiSetting'){
                    window.ecoDesignSettingVm.setViewSceneCallBack(obj.data);
                }
            }
            EcoUtil.addCallBackDialogFunc(callBackDialogFunc,'desingApiSettingConfig');
          },
       ...
       getItemModelCreateApplyFunc(){ 
            getItemModelCreateApplyAjax(this.formOperateId,this.config.modelType,1).then((response)=>{
                if(response.data.status <= 99){
                        ...
                        let _apiSceneId = response.data.remap.api_scene_id;
                        let _viewSceneId = response.data.remap.view_scene_id;
                        

                        /*初始化配置信息*/
                        this.initConfig(_itemOperateId,_itemId,null,_apiSceneId,_viewSceneId);
                        this.createItem();
                }else{
       ...
       /*初始化配置信息*/
       initConfig(_itemOperateId,_itemId,remap,_apiSceneId,_viewSceneId){
            /*初始化缺省项*/
            this.defaultIdArray = [];
       ...
       onBgChange(val){
        ...
        goApiSetting(){
                ...
                if(null == this.sc_entity){
                     _url = '/wh/jsp/version3/flowform/index.html#/selectConnector/'+this.config.itemOperateId+'/0/0/0';
                }else{
                     _url = '/wh/jsp/version3/flowform/index.html#/sceneSetting/'+this.config.itemOperateId+'/'+this.sc_entity.refId+'/'+this.config.apiSceneId;
                }
       ...
       goViewApiSetting(){
                ...
                if(null == this.view_entity){
                     _url = '/wh/jsp/version3/flowform/index.html#/selectConnector/'+this.config.itemOperateId+'/0/0/6';
                }else{
                     _url = '/wh/jsp/version3/flowform/index.html#/viewApiSetting/'+this.config.itemOperateId+'/'+this.view_entity.refId+'/'+this.config.viewSceneId;
                }
                let _height = parent.window.document.getElementById("aside").offsetHeight-180;
...
<style scoped>
.designApiSettingVue .setting{
    margin:10px 20px 50px 20px;
}

.designApiSettingVue .itemVueName{
    Font-weight: bold;
...
.designApiSettingVue .optionsDiv{
    font-size: 13px;
    line-height: 30px;
    height:30px;
...
.designApiSettingVue .optionsDiv .iconfont{
    vertical-align: middle;
}

.designApiSettingVue .optionsDiv .meixuanzhong{
    color:rgba(0, 0, 0, 0.65);
}

.designApiSettingVue .optionsDiv .xuanzhong{
    color:#409eff;
}

.designApiSettingVue .optionsDiv .handle{
    color:#409eff;
    cursor: move;
    font-size: 16px;
}
.designApiSettingVue .optionsDiv .deleteicon{
    color:#409eff;
    cursor: pointer;
    font-size: 16px;
}
.designApiSettingVue .optionsDiv input{
    vertical-align: middle;
    border:0px;
    outline:none;
    color:rgba(0, 0, 0, 0.65);
...
.designApiSettingVue  .addOptions{
