...
import {getSealKgAll,invalidSealKg} from '../../service/service.js'
...
export default{
  ...
  mounted(){
      ...
      this.getSealKgAllFunc();
  },
  methods: {
    addMonitor(){
          let callBackDialogFunc = function(obj){
              if(obj && (obj.action == 'sealKgAddCallBack' || obj.action == 'sealKgEditCallBack')){ //回调的唯一标识符
                window.ecoFrameVm.getSealKgAllFunc()
              }
          }
    ...
    add(){
      if(sysEnv == 1){
            EcoUtil.getSysvm().openDialog('添加印章','/sealManage/index.html#/sealKgAdd/'+this.$route.params.orgId,550,400);
      }else{
            this.$router.push({name:'sealKgAdd',params:{orgId:this.$route.params.orgId,}});
      }
    },
    edit(id){
      if(sysEnv == 1){
            EcoUtil.getSysvm().openDialog('编辑印章','/sealManage/index.html#/sealKgEdit/'+id,550,400);
      }else{
            this.$router.push({name:'sealKgEdit',params:{id:id}});
      }
    },
    ...
    del(id){
          ...
          let confirmYesFunc = function(){
              invalidSealKg(id).then((response)=>{
                  _that.$message({type: 'success', message: '删除成功!'});
                  _that.getSealKgAllFunc();
              }).catch((error)=>{
    ...
    getSealKgAllFunc(){
        ...
        getSealKgAll(orgId).then((response)=>{
