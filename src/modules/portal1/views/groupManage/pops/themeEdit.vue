...
import {updateTitle,getTitleSingle} from '@/modules/portal1/service/service.js'
import {mapMutations} from 'vuex'
import themeVue from '../components/theme.vue';
export default{
  name:'themeEdit',
  components:{
    ecoLoading,
    ...
    save(){
      this.$refs['form'].validate((valid) => {
          if (valid) {
            let id = this.$route.params.id;
            this.$refs.ecoLoadingRef.open();
            updateTitle(id,this.form).then((res)=>{
              if (res.data&&res.data.id){
                try {
