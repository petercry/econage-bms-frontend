...
export default {
  ...
  methods: {
      ...
      loadJs(url) {
        ...
        if (window.CKEDITOR && window.CKEDITOR.status == 'loaded') {//如果已加载ckeditor 则直接引用
          that.initCkeditor();
          return;
        }
    ...
    initCkeditor() {
      ...
      CKEDITOR.replace(this.$refs['ckeditor'], {height: this.height, width: this.width, toolbar: this.toolbar});
      if (this.content) {
   ...
   transData(value) {
      return value.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, function (match, capture) {
        if (capture.indexOf('http') > -1 || capture.indexOf('https') > 1 ) {
          return match;
        }
   ...
   getCkeditorData() {
        ...
        return value.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, function (match, capture) {
          let fileHeaderId = capture.substring(capture.indexOf('file-header-Id=') + 15);
