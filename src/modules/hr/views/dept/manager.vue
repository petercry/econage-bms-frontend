                        ...
                        <div class="dept_user_item" v-for="(item, index) in itemList" :key="index">
                          <el-row :gutter="20">
                                <el-col :span="12">
                                    ...
                                    <el-select v-model="item.deptId"  style="width:100%;"  class="select" placeholder="请选择分支机构">
                                          <el-option
...
import {getDeptManager,editDeptManager,getDetpAllBranchDepView } from '../../service/service.js'
  ...
  methods: {
        ...
        getDetpAllBranchDepViewFunc(){
              getDetpAllBranchDepView().then((response)=>{
                  let _departments = [];
                  // _departments.push({id:'-100',name:'无'});
        ...
        handleTabClick(val){
            if(val == 'watch'){
                this.$router.push({name:'deptWatcher'});
            }
        },

        getDeptManager(){
            getDeptManager().then(res=>{
      ...
      save(){
          ...
          for(let i = 0;i<arr.length;i++){
              if(arr[i].deptId == null  ){
                  EcoMessageBox.alert('请选择第'+(i+1)+'行 分支机构');
                  return ;
              }else if(arr[i].userId == null){
                  EcoMessageBox.alert('请选择第'+(i+1)+'行 授权人员');
                  return ;
              }
          }

        
          editDeptManager(arr).then(res=>{
