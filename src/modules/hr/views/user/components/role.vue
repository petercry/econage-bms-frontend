...
export default{
  ...
  methods: {
    ...
    addRole(){
          ...
          if(sysEnv == 1){
               EcoUtil.getSysvm().openDialog('角色添加','/hr/index.html#/userRoleAdd/'+userId+'/'+deptId,700,400);
          }else{
              this.$router.push({name:'userRoleAdd',params:{userId:userId,deptId:deptId}})
          }
    },
    ...
    editRole(id){
          let userId = this.$route.params.userId; 
           if(sysEnv == 1){
               EcoUtil.getSysvm().openDialog('角色编辑', '/hr/index.html#/userRoleEdit/'+userId+'/'+id,700,400);
           }else{
