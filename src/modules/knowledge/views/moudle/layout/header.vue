    ...
    <el-row style="padding:12px 10px;background-color:#fff;">
      <el-col :span="24">
        ...
        <el-button
          ...
          size="small"
          class="left-float"
          @click.native="uploadMulti"
        >批量上传</el-button>
        <el-button
          type="primary"
          size="small"
          class="left-float"
          @click.native="handleSort"
        >排序</el-button>
        <el-button
          size="small"
          class="left-float"
...
import { deleteFolderOrFile,getListDataByName } from '@/modules/knowledge/api/knowledge.js'
export default {
  name: 'knHeader',
  components: {
    ecoToolTitle
  ...
  methods: {
    ...
    uploadFile() {
      ...
      if (sysEnv !== 1) {
        this.$router.push({ name: 'fileUpload', params: { id: this.id, activeid: this.activeId } })
      } else {
        let url = '/knowledge/index.html#/fileUpload' + '/' + this.id + '/' + this.activeId;
        EcoUtil.getSysvm().openDialog('上传文档', url, 600, 600, '12vh');
      }
    },
    addNewFolder() {
      if (this.activeId == '') {
