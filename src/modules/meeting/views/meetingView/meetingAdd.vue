                    ...
                    <el-row>
                        ...
                        <el-col :span="12">
                            <el-form-item label="会议室地点" prop="roomName" :rules="[{required: true, message:'会议室地点必须填写',trigger: 'change'}]">
                                <el-input v-model="baseInfo.roomName" readonly @click.native="selectMeetingRoom" style="cursor:pointer">
...
<script>
  ...
  import {addMeetingAjax,getItemFetchId,getNoticeFormEnum,getEnumSelectEnabled} from '../../service/service'
  ...
  export default {
      ...
      created(){
           ...
           if( _storeKey){
                try{
                    ...
                    if(_storeData){
                        this.baseInfo.startTime = _storeData.startTime;
                        this.baseInfo.endTime = _storeData.endTime;
                        this.baseInfo.roomId = _storeData.roomId;
                        this.baseInfo.roomName = _storeData.roomName;
                    }
                }catch(e){}
           }
      ...
      methods: {
            ...
            // 表格添加行
            addTableLine(){
                this.externalPersonData.push({name:'',email:''})
            },
                      ...
                      if (valid) {
                            ...
                            for(let i =0;i<this.externalPersonData.length;i++){
                                _confereeExternals.push({
                                    ...
                                    emailAddr:this.externalPersonData[i].email
                                })
                            }
                            
                            addMeetingAjax(that.baseInfo,_confereeExternals).then((response)=>{
