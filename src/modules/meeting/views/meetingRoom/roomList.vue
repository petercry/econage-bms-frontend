      ...
      <div class="layoutInnerAbsoluteDiv">
          ...
          <eco-content top="0px" height="60px" type="tool">
              <el-row style="padding:12px 10px;background-color:#fff;">
                    <el-col :span="8" >
                        <eco-tool-title style="line-height: 34px;" :title="'会议室（'+params.total+'）'"></eco-tool-title>
                    </el-col>
          ...
          <eco-content top="59px" v-show="this.searchShow">
              <div class="searchBox">
                  <span>会议室名称：</span>
                  <div class="itemInput">
                      <el-input  placeholder="请输入会议室名称" @keyup.enter.native="searchListFunc" v-model="params.name"></el-input>
                  </div>

                  <span>创建日期：</span>
                  <el-date-picker
                    v-model="params.createDateFrom"
                    type="date"
...
<script>
import {getRoomListAjax,deleteMeetingRoomAjax} from '../../service/service.js'
    ...
    methods: {
        ...
        listAction(){
                let callBackDialogFunc = function(obj){
                    if(obj && obj.action == 'updateRoomAddBack'){
        ...
        getListFunc(){
              getRoomListAjax(this.params).then((response)=>{
        ...
        goAddPage(){
            if(sysEnv == 1){
                let url = '/meeting/index.html#/roomAdd';
                EcoUtil.getSysvm().openDialog('会议室新增',url,750,500,'8vh');
            }else{
        ...
        editItem(id){
               if(sysEnv == 1){
                  let url = '/meeting/index.html#/roomEdit/'+id;
                  EcoUtil.getSysvm().openDialog('会议室编辑',url,750,500,'8vh');
              }else{
          ...
          deleteItem(id){
              ...
              let confirmYesFunc = function(){
                  let _ids = [];
                  _ids.push(id);
                  deleteMeetingRoomAjax(_ids).then((response)=>{
                      that.$message({
                              showClose: true,
                              message:'删除成功',
                              type: 'success',
          ...
          refreshFunc(){
                ...
                this.params.createDateFrom = null;
