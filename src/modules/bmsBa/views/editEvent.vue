<template>
    <div >
        ...
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" @open="cleanDialog()" :destroy-on-close="false" ref="dialog" :close-on-click-modal="false" :close-on-press-escape="false" :append-to-body="true" class="trivialDialog">
          <ecoFileUploadPopup v-if="dialogTab=='addFile'" modular="bmsBaEvent" :modularInnerId="''+this.eventId" ref="addFileWin"></ecoFileUploadPopup>
...
import { getBaDetail,getEnumText,getEventDetail,editBaEventAjax, formatFileSize,deleteFileDataServer,getEventFileList } from "@/modules/bmsBa/service/service.js";
    ...
    getEventInfo(eventId){
        ...
        getEventDetail(eventId).then((response)=>{
            if (response.data&&response.data.id){
                this.eventInfoObj = response.data;
    ...
    save(){
      this.$refs['myform'].validate((valid) => {
          if (valid) {
            ...
            //return;
            this.$parent.$parent.openLoading();
            editBaEventAjax(this.eventId,this.setSaveData(this.eventInfoObj)).then((res)=>{
