<template>
  <div class="">
    <div class="kn-header" >
      ...
      <div>
        添加菜单
        <ecoActionBtn :ecoActionBtnFunc="addCustomSysMenuItem">
...
import {getMenuType,getModularList,getI18nList,addCustomSysMenuItem} from '@/modules/menuFacade/service/service.js'
...
export default {
  ...
  methods:{
    ...
    init(){
      this.form.id='';
      this.form.type='';
      let treeSelected = this.sysTree.getCurrentNode();
    ...
    addCustomSysMenuItem(){
      this.$refs['form'].validate((valid) => {
          if (valid) {
            this.$refs.ecoLoadingRef.open();
            addCustomSysMenuItem(this.form).then((res)=>{
              if (res.data&&res.data.id){
                this.$message({type: 'success',message: '添加成功！'});
                // this.sysTree.$parent.getCustomMenuTree(res.data.id);
                let obj =res.data;
                if (obj){
                  // let levelId = this.sysTree.getCurrentNode()?(this.sysTree.getCurrentNode().levelId+1):1;
                  // this.addTreeNode(this.form.parentId,{
                  //   name:obj.name,
                  //   id:obj.id,
                  //   // levelId:levelId
                  // })
                  let pNode = this.sysTree.getNode(this.form.parentId);
                  if (pNode.data.children){
                    pNode.data.children.push(obj)
                  }else{
                    pNode.data.children=[obj]
                  }
                  pNode.updateChildren();
    ...
    // //添加节点的方法根据code排序
    addTreeNode(parent_id,obj){
      let pNode = this.sysTree.getNode(parent_id);
      let pChild = [];
      if (pNode.data.children){
        pChild = pNode.data.children.map(item=>{
          item.code =parseInt(item.desc.replace(/^[\s\S]+\(([0-9]+)\)$/,'$1'));
          return item;
        })
      //   let obj ={desc: "首页 (92)",id: 253,levelId: 1};
        let objCode = parseInt(obj.name.replace(/^[\s\S]+\(([0-9]+)\)$/,'$1'));
        let index=-1;
        for (let i=0;i<pChild.length;i++){
          if (pChild[i].code>objCode){
            index=i;
            break;
          }
        }
        if(index==-1){
          ...
          pNode.data.children.splice(index,0,obj)
        }
        pNode.updateChildren();
      }else{
        pNode.data.children=[obj]
