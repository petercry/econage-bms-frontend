...
import {baseUrl,sysEnv} from '@/modules/menuFacade/config/env'
...
export const loginAjax = function(){
  return axios.post(baseUrl+'/login',qs.stringify({
      username:'econage',
      password:'4d3ba6604a8cb93c6c891b74cf4a6f583c22330eed03610d29833af664c89503',
...
export const getMenuType = function(){
  return axios.get(baseUrl+'/facade-menu/menu-type-map',{
    params:{
      time:new Date().getTime()
    }
  })
...
export const getCustomMenuDetail = function(id){
  return axios.get(baseUrl+'/facade-menu/'+id,{
    params:{
      time:new Date().getTime()
    }
  })
...
export const addCustomSysMenuItem= function(data){
  return axios.post(baseUrl+'/facade-menu',{
    id:data.id,   //菜单id
    name:data.name,   //菜单名称
...
export const updateCustomSysMenuItem= function(data){
  return axios.put(baseUrl+'/facade-menu/'+data.id,{
    menuId:data.id,
    name:data.name,   //菜单名称
    i18nKey:data.i18nKey,   //国际化key
    parentId:data.parentId, //上级节点
...
export const deleteCustomSysMenuItem= function(id){
  return axios.delete(baseUrl+'/facade-menu/'+id,{
    data:qs.stringify({
      time:new Date().getTime()
    },{arrayFormat:'repeat'}),
    traditional:true})
...
export const dndCustomSysMenuItem= function(data){
  return axios.patch(baseUrl+'/facade-menu/'+data.id+'/dnd',qs.stringify({
