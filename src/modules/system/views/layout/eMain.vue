<template>
    <div :class="{mainLeftMax:toggleFlag==1}"  class="mainVue" v-bind:style="eMainStyle">
        <div class="tab" v-bind:class="['mainTab'+tabsArr.length,'frameMainPage'+frameMainPage]" @contextmenu.prevent="findTarget">
            <el-tabs v-model="activeTabKey" ref="tabs" type="border-card" closable @tab-remove="removeTab" @tab-click="clickTab">
    ...
    <ecoDialog ref="ecoDialog1" :id="'ecoDialog1'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>
    <ecoDialog ref="ecoDialog2" :id="'ecoDialog2'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>
    <ecoDialog ref="ecoDialog3" :id="'ecoDialog3'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>
    <ecoDialog ref="ecoDialog4" :id="'ecoDialog4'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>
    <ecoDialog ref="ecoDialog5" :id="'ecoDialog5'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>
    <ecoDialog ref="ecoDialog6" :id="'ecoDialog6'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>
    <ecoDialog ref="ecoDialog7" :id="'ecoDialog7'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>
    <ecoDialog ref="ecoDialog8" :id="'ecoDialog8'" @closeDialog="closeDialogCallBack" @closedDialog="closedDialogCallBack" v-dialogDrag></ecoDialog>

        <context-menu class="right-menu"
            :target="contextMenuTarget"
            :show="contextMenuVisible"
            @update:show="(show) => contextMenuVisible = show">
            <a href="javascript:void(0);" @click="closeTab">关闭</a>
            <a href="javascript:void(0);" @click="closeOthers">关闭其他标签</a>
            <a href="javascript:void(0);" @click="refreshTab">刷新</a>
        </context-menu>

        <div class="toggleMenu" v-bind:class="['toggle'+toggleFlag]" @click="toggleMenuFunc" v-if="menuToggleShow">
            <span v-html="'&nbsp;'"></span><br/><br/><span v-html="'&nbsp;'"></span>
        </div>
...
<script>
  ...
  import {getSSOLinkAjax,getWFViewOperateId,getPublicSettingUnion,getWFOperateId} from '@/modules/system/service/service.js'
  ...
  export default {
   ...
   computed:{
    ...
    mounted(){
        window.sysvm = this;
        this.contextMenuTarget =  this.$refs['tabs'].$el.children[0].children[0].children[0].children[0];
        this.init();
    ...
    methods: {
      ...
      init(){
            ...
            let r_func_Str =  sessionStorage.getItem('toInitPage');
            if (r_func_Str){
                let toInitPageTitle =  sessionStorage.getItem('toInitPageTitle');
                let nextMenu2 = {}
                nextMenu2.desc = toInitPageTitle;
                nextMenu2.r_func = r_func_Str;
                this.doTab(nextMenu2);
                sessionStorage.removeItem('toInitPage');
                sessionStorage.removeItem('toInitPageTitle');
            }
      ...
      logout(){
            ...
            if (window.sysSetting.loginWarnPage){
             window.location.replace(window.sysSetting.loginWarnPage);
            }else{
      ...
      findTarget(e){
            ...
            if(this.selectedTab.classList.value.indexOf('el-tabs__item')>-1){
      ...
      doTab(menuTabObj){
          ...
          if(_funcObj && _funcObj.fullScreen){
              ...
              if(this.tabsArr.length>=12){
                ...
                if(menuTabObj.funcObj.menuTarget == 'APP_SSO'){
      ...
      getAndChangeSSO(tabObj){
            let _paramName = tabObj.funcObj.paramName;
            let _paramVal = tabObj.funcObj.paramVal;

            getSSOLinkAjax(_paramName,_paramVal).then((response)=>{
      ...
      actionTab(menuTabObj){
          if(menuTabObj.funcObj.menuTarget == 'IFRAME' || menuTabObj.funcObj.menuTarget == 'WEB' ||  menuTabObj.funcObj.menuTarget == 'SSO' ){
              ...
              this.$nextTick(function(){
                  this.$refs[this.activeTabKey][0].onload = function(e){
                     this.contentWindow.document.onclick = function(e){
              ...
              this.$router.replace({name:routerName,params:routerParams}).catch((err)=>{});;
              ...
              let activeTabKeyIframe = null;
              activeTabKeyIframe = document.getElementById(this.activeTabKey+'ifm');
              activeTabKeyIframe.src = window.location.origin + "/#/"+ this.$route.path.replace('/work/','');
          }else{
              this.vueShow = false;
              this.$router.replace({name:'nonePage'}).catch((err)=>{});;
          }
      ...
      removeTab(targetName) {
          if(targetName == "home" || targetName == 'contentPage'){
              return ;
          }
    ...
    hasTabKey(tabKey){
     ...
     getIframeSrc(tabObj){
          if(tabObj.funcObj && tabObj.funcObj['menuTarget'] == 'WEB'){
              ...
              return url;
               ...
               let url = this.$route.path.replace('/work/','');
      ...
      callBackDialogFunc(obj){
          ...
          if(obj && obj.callTab){
            ...
            if(false&& window.fullScreenVm.fullScreenIfmShow){
                    ...
                    /*回调当前页签的 事件*/
                    let activeTabKeyIframe = null;
                    let activeTabKeyIframeFull = null;
                    if(window.fullScreenVm.fullScreenIfmShow){
                        /*回调全盘的事件*/
                        activeTabKeyIframeFull = document.getElementById('fullScreenIfm');
                    }
                    activeTabKeyIframe = document.getElementById(this.activeTabKey+'ifm');
                    ...
                    if(activeTabKeyIframeFull){
                        try{
                                if(activeTabKeyIframeFull.contentWindow){
                                    console.log("调用activeTabKeyIframe中callBackDialogFunc ");
                                    if(obj && obj.action == 'userSelectCallBack'){
                                        if( activeTabKeyIframeFull.contentWindow.callBackUserSelectDialogFunc){
                                            activeTabKeyIframeFull.contentWindow.callBackUserSelectDialogFunc(obj);
                                        }
                                    }else{
                                        if( activeTabKeyIframeFull.contentWindow.callBackDialogFunc){
                                            activeTabKeyIframeFull.contentWindow.callBackDialogFunc(obj);
                                        }
                                    }
                                }
                        }catch(e){
      ...
      closedDialogCallBack(callObj){
         try{
             this.$refs['ecoDialog'+(this.openedDialog+1)].$el.querySelector('.el-dialog').style.left = null;
             this.$refs['ecoDialog'+(this.openedDialog+1)].$el.querySelector('.el-dialog').style.top = null;
         }catch(e){}
      },
     ...
     openFullScreenDialog(url){
        ...
        let _height =   document.getElementById("aside").offsetHeight;
      ...
      showTopFormContent(wf_id){
       ...
       showWFViewFullScreen(wf_id){
            ...
            getWFViewOperateId(_wfId,formView,_ccId).then((response)=>{
                    if(response.data.status <= 99){
                        let tabObj = {};
                        let goPage = "flowform/index.html#/wfViewDetail/"+wf_id+"/"+response.data.operate_id;
                        tabObj.desc = '流程查看';
     ...
     selectMyFlowFunc(){
      ...
      getEcoCookie(name){
          ...
          let theme = localStorage.getItem(cookieName);
          return theme;
      },
      ...
      /*关闭全局窗口*/
        ...
        closeOthers(){
            ...
            key = key.replace('tab-','');
            // console.log(key)
            this.tabsArr = this.tabsArr.filter(tab => tab.tabKey === key || tab.tabKey === 'home' || tab.tabKey === 'contentPage');
            // console.log(this.tabsArr)
            let activeTabKeyIframe = null;
            this.activeTabKey = key;
            if(key === 'contentPage' || key === 'home'){
                this.actionTab(this.tabsArr[0]);//刷新

            }else{
                this.actionTab(this.tabsArr[1]);//刷新第二个标签
            }
            activeTabKeyIframe = document.getElementById(this.activeTabKey+'ifm');
                if(activeTabKeyIframe){
                    try{
      ...
      openCommentDialogEnv(obj){
          ...
          this.openDialog(obj.title || '评论',url,600,500,'15vh');
      },
    ...
    watch:{
       ...
       themeStatus(value,oldValue){
        ...
        contextMenuVisible(value,oldValue){
           if(!value){
               this.selectedTab = null;
           }
       },
...
.mainVue .mainTab1.frameMainPagefalse .el-tabs{
    display:none;
}

.mainVue .mainTab1 .el-tabs__content{
    top:0px !important;
    z-index: 2;
}
