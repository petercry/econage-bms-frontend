...
<el-card  :body-style="{ padding: '0 20px'}" style="" shadow='never'>
  <div style="position:relative;">
    <div class="homeTitle border">待办业务
        <span class="more cpointer" @click="goMore()">({{toDoParams.total}})</span>

        <span class="fr" v-if="showWFPortalToDoTab">
            <el-radio-group v-model="toDoParams.cosignFlag" @change="handleTabClick">
                <el-radio-button label="">全部待办</el-radio-button>
                <el-radio-button label="D">转交待办</el-radio-button>
                <el-radio-button label="C">征询待办</el-radio-button>
            </el-radio-group>
        </span>

    </div>
    <div style="padding:20px 0;height:350px;">
      <el-card v-for="item in toDoList" :key="item.id" class="wfItem cpointer" :body-style="{ padding: '12px 20px 4px'}" shadow="hover" @click.native="goDetail(item.id,item.defFieldId,item.requestDesc)">
        <div class="info">
...
export default {
  ...
  created() {
        if(window.sysSetting && window.sysSetting.showWFPortalToDoTab ){
            this.showWFPortalToDoTab = window.sysSetting.showWFPortalToDoTab;
        }
  ...
  methods: {
        ...
        goDetail(wfId,defFieldId,requestDesc){
