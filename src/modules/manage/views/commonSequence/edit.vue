...
import {getCommonSequenceSingle,editCommonSequence,getCommonSequenceFormuiaType,getCommonSequenceIdxRestType,previewCommonSequence} from '../../service/service.js'
import {mapMutations} from 'vuex'
export default{
  name:'editCommonSequence',
  components:{
  ...
  methods: {
    getData(){
      ...
      getCommonSequenceSingle(id).then(res=>{
        if (res.data){
          this.form.prefix = res.data.prefix;
          this.form.name = res.data.name;
          this.form.formulaType = res.data.formulaType;
          this.form.formulaSuffix = res.data.formulaSuffix;
          this.form.startIdx = res.data.startIdx;
          this.form.length = res.data.length;
          this.form.isFixLengthShow = res.data.fixLengthShow;
          this.form.suffix = res.data.suffix;
          this.form.idxResetType = res.data.idxResetType;
          this.form.version = res.data.version;
          this.previewResult = res.data.ticketPreview
        }
      }).catch(e=>{})
    },
    ...
    save(){
      this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            editCommonSequence(id,this.form).then((res)=>{
              ...
              try {
                if (res.data&&res.data.id){
                  ...
                  let doObj = {}
                  doObj.action = 'commonSequenceEditCallBack';
                  doObj.close = true;
