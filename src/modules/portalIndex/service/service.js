...
import {baseUrl,ecoPubAuthToken,sysEnv} from '@/modules/portalIndex/config/env'
import qs from 'qs';
//全局设置超时时间 120s
...
//主题------事项列表页面主题列表
export const getTitleList4select = function(){
...
export const getGroupItemSelectViewList = function(data){
  return axios.get(baseUrl+'/portal1/group-item-view/select-view',{
    params:{
      ...
      attr2:data.attr2,
      itemAttr6:data.itemAttr6,
      myFavoriteQuery:data.myFavoriteQuery||null,
      time:new Date().getTime()
    }
  })
}
...
//事项------事项列表
export const getItemList = function(data){
  return axios.get(baseUrl+'/portal1/item/view-pagination',{
    params:{
      titleGroupId:data.titleGroupId||'',
      myFavoriteQuery:data.myFavoriteQuery||null,
      attr2:data.attr2||null,
      attr1:data.attr1||null,
      page:data.page,
      rows:data.rows,
      sort:data.sort,
      order:data.order,
...
//事项收藏
export const addItemFavorite= function(itemId){
  return axios.post(baseUrl+'/portal1/item-favorite/'+itemId,{
  },{
    headers: {'Content-Type': 'application/json;;charset=UTF-8'},
    traditional:true})
}
...
//事项取消收藏
export const delItemFavorite = function(itemId){
  return axios.delete(baseUrl+'/portal1/item-favorite/'+itemId,{
    params:{
      time:new Date().getTime()
    }
  })
...
export const getDingSync = function(){
  return axios.get(baseUrl+'/org/dingding-sync',{
    params:{
      time:new Date().getTime()
    }
  })
...
export const configDingSync= function(){
  return axios.post(baseUrl+'/org/dingding-sync',{
  },{
    headers: {'Content-Type': 'application/json;;charset=UTF-8'},
    traditional:true})
}
...
//待办项-登陆人员待办列表
export const getTodoAssigneeList = function(status){
  return axios.get(baseUrl+'/portal/personal-todo/assignee-list',{
    params:{
      status:status,
      time:new Date().getTime()
    }
