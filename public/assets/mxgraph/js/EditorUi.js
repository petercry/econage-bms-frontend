...
EditorUi = function (editor, container, lightbox) {
	...
	_UNDO = EditorUi.prototype.undo.bind(this)
	...
	if (!graph.standalone) {
		...
		if (this.toolbar != null) {
			let update = mxUtils.bind(this, function () {
				...
				let fs = String(graph.currentVertexStyle['fontSize'] || '14');
 ...
 * screen widths <= 640px.
 */
EditorUi.prototype.hsplitPosition = (screen.width <= 640) ? 118 : ((urlParams['sidebar-entries'] != 'large') ? 130 : 240);
...
EditorUi.prototype.init = function () {
	...
	graph.dblClick = function (evt, cell) {
		if (this.isEnabled()) {
				//修改 只有在PRO模式下
			if (cell == null && ui.sidebar != null && !mxEvent.isShiftDown(evt) && versionType === 'PRO') {
				mxEvent.consume(evt);
	...
	graph.click = function (evt) {
		if (this.isEnabled()) {
			...
			if (versionType.indexOf('VCA') > -1 && versionType != 'VCA-L4') {
				let cell = evt.getCell();
				if (!cell || !cell.compType) {
					return hideDialogFixed();
				}
				if (cell.compType == 'vca.linkPlus') {
					return goVcaLink(cell);
				} else {
					return showVcaAttr(cell);
				}
...
EditorUi.prototype.getCellsForShapePicker = function (cell) {
	...
	// 修改箭头显示的添加组件
	return [
					createVertex('rounded=0;whiteSpace=wrap;html=1;container=1;recursiveResize=0;isTask=1', 120, 60, ''),
					createVertex('shape=datastore;whiteSpace=wrap;compType=ecoBpmn.system;html=1;recursiveResize=0;isTask=1;', 80, 80, ''),
					createVertex('html=1;whiteSpace=wrap;aspect=fixed;compType=decision;shape=isoRectangle;', 120, 80, '')
				 ]
};
...
EditorUi.prototype.createUi = function () {
	...
	// Creates toolbar
	this.toolbar = (this.editor.chromeless || readonlyXml) ? null : this.createToolbar(this.createDiv('geToolbar'));
	...
	// HSplit
	if (this.sidebar != null && !readonlyXml) {
