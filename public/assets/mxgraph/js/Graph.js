...
Graph.prototype.getPageLayout = function () {
	...
	if (bounds.width == 0 || bounds.height == 0) {
		...
		let x0 = Math.floor(Math.ceil(bounds.x / this.view.scale -
			this.view.translate.x) / size.width);
		let y0 = Math.floor(Math.ceil(bounds.y / this.view.scale -
			this.view.translate.y) / size.height);
		let w0 = Math.ceil((Math.floor((bounds.x + bounds.width) / this.view.scale) -
			this.view.translate.x) / size.width) - x0;
		let h0 = Math.ceil((Math.floor((bounds.y + bounds.height) / this.view.scale) -
 ...
 * 修改出现箭头的时间，默认500
 */
HoverIcons.prototype.updateDelay = 0;
 ...
 * 修改出现箭头的时间，默认140
 */
HoverIcons.prototype.activationDelay = 0;
 ...
 * Up arrow.
 */
HoverIcons.prototype.inactiveOpacity = 100;
...
HoverIcons.prototype.click = function (state, dir, me) {
	...
	//修改
	if (state.cell.id === 'strategy' || state.cell.id === 'business' || state.cell.id === 'support' || state.cell.id === 'workflow') {
		return false;
	}
	// if (state.cell.compType && state.cell.compType !== 'swimlane' ) {
	// 	return false;
	// }
...
HoverIcons.prototype.update = function (state, x, y) {
	if (!this.graph.connectionArrowsEnabled || (state != null &&
		mxUtils.getValue(state.style, 'allowArrows', '1') == '0') || (state != null && (state.cell.id === 'strategy' || state.cell.id === 'business' || state.cell.id === 'support' || state.cell.id === 'workflow'))) {
		this.reset();
	} else {
						...
						if (parent != null && row != null &&
							...
							if (rows.length > 0) {
								...
								for (var j = 0; j < cols.length; j++) {
									...
									let geo2 = this.getCellGeometry(sourceCols[j]);
