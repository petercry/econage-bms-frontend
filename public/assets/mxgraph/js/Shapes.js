...
(function () {
	...
	CubeShape.prototype.paintVertexShape = function (c, x, y, w, h) {
		...
		let op2 = Math.max(-1, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'darkOpacity2', this.darkOpacity2))));
		c.translate(x, y);
...
//		var tp = mxUtils.getValue(this.style, 'tabPosition', this.tabPosition);
	...
	// StepFromTo shape
	function StepFromToShape() {
		mxActor.call(this);
	}
	mxUtils.extend(StepFromToShape, mxActor);
	StepFromToShape.prototype.size = 0.2;
	StepFromToShape.prototype.fixedSize = 20;
	StepFromToShape.prototype.isRoundable = function () {
		return true;
	};
	StepFromToShape.prototype.redrawPath = function (c, x, y, w, h) {
		let fixed = mxUtils.getValue(this.style, 'fixedSize', '0') != '0';
		let s = (fixed) ? Math.max(0, Math.min(w, parseFloat(mxUtils.getValue(this.style, 'size', this.fixedSize)))) :
			w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		let arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(0, 0), new mxPoint(w - s, 0), new mxPoint(w, h / 2), new mxPoint(w - s, h),
												new mxPoint(0, h), new mxPoint(0, h)], this.isRounded, arcSize, true);
		c.end();
	};

	mxCellRenderer.registerShape('stepFromTo', StepFromToShape);
	// Hexagon shape
	...
	if (typeof mxVertexHandler !== 'undefined') {
		...
		let handleFactory = {
			...
			'stepFromTo': createDisplayHandleFunction(StepFromToShape.prototype.size, true, null, true, StepFromToShape.prototype.fixedSize),
	...
	StepShape.prototype.constraints = [new mxConnectionConstraint(new mxPoint(0.25, 0), true),
									...
									new mxConnectionConstraint(new mxPoint(1, 0.5), true),
									new mxConnectionConstraint(new mxPoint(1, 0.75), true)];
	StepFromToShape.prototype.constraints = [new mxConnectionConstraint(new mxPoint(0.25, 0), true),
		new mxConnectionConstraint(new mxPoint(0.5, 0), true),
		new mxConnectionConstraint(new mxPoint(0.75, 0), true),
		new mxConnectionConstraint(new mxPoint(0.25, 1), true),
		new mxConnectionConstraint(new mxPoint(0.5, 1), true),
