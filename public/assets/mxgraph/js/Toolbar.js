...
Toolbar.prototype.init = function () {
	...
	let viewMenu = this.addMenu('', mxResources.get('zoom'), true, 'viewZoom', null, true);
	...
	if (sw >= 550) {
		...
		if (versionType === 'VCA-L0' || versionType === 'VCA-L1' || versionType === 'VCA-L2' || versionType === 'VCA-L3') {
			var elts = this.addItems(['-', 'sort']);
			elts[1].setAttribute('title', '排序');
		}
	}

	if (sw >= 740) {
		this.addItems(['-', 'fillColor','fontColor']);
	...
	if (sw >= 400) {
		...
		if (sw >= 440) {
			var elts = this.addItems(['-', 'dashed','solid']);
			elts[1].setAttribute('title', '虚线');
			elts[2].setAttribute('title', '实线');
			this.addSeparator();
        ...
        // this.edgeTypeMenu = this.addMenuFunction('',mxResources.get('pattern'),false,mxUtils.bind(this, function (menu) {
        ...
        //     let item = this.editorUi.menus.edgeStyleChange(menu, '', keys, values, 'geIcon', null);
        ...
        //     return item;
        // });
        //this.addDropDownArrow(this.edgeTypeMenu, 'geSprite-noarrow', 44, 50, 0, 0, 22, -4);
...
Toolbar.prototype.createButton = function (classname) {
	...
	if (classname != null) {
		if (classname === 'geSprite-dashed') {
			inner.className = 'ecoDashed';
		} else if (classname === 'geSprite-solid') {
			inner.className = 'geSprite geSprite-noarrow';
		} else {
