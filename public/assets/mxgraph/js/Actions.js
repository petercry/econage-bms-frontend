...
Actions.prototype.init = function () {
 ...
 mxClipboard.cut(graph);
	...
	function deleteCells(includeEdges) {
		...
		let deleteCells = cells.filter(element => element.id !== 'strategy' && element.id !== 'business' && element.id !== 'support' && element.id !== 'workflow')
		console.log(deleteCells)
		let parents = graph.model.getParents(cells);
			graph.removeCells(deleteCells, true);
	...
	this.addAction('delete', function (evt) {
		let cells = graph.getDeletableCells(graph.getSelectionCells());
		let ids = cells.map(function (item) {
			return item.compId || '-1';
		});
		deleteStandardRequest(ids,deleteCells);
	}, null, null, 'Delete');
	this.addAction('sort', function () {
			showSort();
	}, null, null, 'sort');
 ...
 return ui.formatWidth > 0;
	...
	action = this.addAction('outline', mxUtils.bind(this, function () {
		if (this.outlineWindow == null) {
			...
			this.outlineWindow = new OutlineWindow(ui, document.body.offsetWidth - 160, 460, 140, 140);
