...
CKEDITOR.editorConfig = function ( config ) {
  ...
  //  config.height = '300px';
    ...
    config.fileTools_requestHeaders = {
      ...
      'eco-auth-token': window.sessionStorage.getItem('ecoToken') || window.EcoToken.getToken()
    };
	...
	config.toolbar_ScheduleOP = [ [ 'Styles', 'Format', 'Font', 'FontSize' ],
			...
			[ 'TextColor', 'BGColor' ] ];
	config.toolbar_RSF = [[ 'Styles', 'Format', 'Font', 'FontSize','lineheight' ],
			[ 'Bold', 'Italic', 'Underline', 'StrikeThrough' ],
			[ 'Undo', 'Redo' ],
			[ 'Link', 'Unlink', 'Anchor' ],
			[ 'JustifyLeft', 'JustifyCenter', 'JustifyRight', 'JustifyFull' ],
			[ 'Indent', 'Outdent', 'NumberedList', 'BulletedList' ],
			[ 'TextColor', 'BGColor', '-', 'Image', 'Html5video','Table','HorizontalRule','ecoFile','filetools']];
    ...
    config.extraPlugins += ',image2,ecoFile,filetools';
