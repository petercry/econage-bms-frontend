	...
	CKEDITOR.plugins.add( 'filetools', {
		...
		beforeInit: function( editor ) {
			...
			editor.on( 'fileUploadRequest', function( evt ) {
				...
				var uploadUrl = fileLoader.uploadUrl+"?modularInnerId="+new Date().getTime();
				fileLoader.xhr.open( 'POST', uploadUrl, true );
			...
			editor.on( 'fileUploadRequest', function( evt ) {
				...
				$formData.append( 'fileId', CKEDITOR.tools.getCsrfToken() );
				$formData.append( 'targetFileName', requestData.upload.name );
			...
			editor.on( 'fileUploadResponse', function( evt ) {
				...
				try {
					...
					if ( !response.id ) {
						...
						console.log(EcoToken.fileManager.getTempToken())
						data.url = '/api/file-manager/safe/preview?mt-token='+EcoToken.fileManager.getTempToken()+'&file-header-Id='+response.id;
	...
	FileLoader.prototype = {
		...
		upload: function( url, additionalRequestParameters ) {
			...
			if ( !url ) {
				...
				var that = this;
				this.uploadChunkRequest().then(function(){
					console.log('分块上传完成')
					if ( that.editor.fire( 'fileUploadRequest', { fileLoader: that, requestData: requestData } ) ) {
		...
		uploadChunkRequest:function(){
			var fileLoader = this;
			return new Promise(function(resolve, reject ){
				try{
					var xhr = new XMLHttpRequest();
					var SLICESIZE = 5 * 1024 * 1024;
					var totalsize = fileLoader.total;
					var chunks = Math.ceil(totalsize/SLICESIZE);
					var fileBlob = dataToFile(fileLoader.data);
					var start = 0;
					var configXhrHeaders = fileLoader.editor.config.fileTools_requestHeaders;
					while(start<chunks){
						
						let url = "/api/file-manager/uploader/chunk/save/ckeditor?fileId="+CKEDITOR.tools.getCsrfToken()+"&name="+fileLoader.fileName+"&chunk="+start+"&chunks="+chunks+"&time="+new Date().getTime();
						var chunkBlob = fileBlob.slice(start*SLICESIZE, (start+1)*SLICESIZE>totalsize?totalsize:(start+1)*SLICESIZE);
						start++;
						xhr.open('post', url, false);
						if ( configXhrHeaders ) {
							for ( var header in configXhrHeaders ) {
								xhr.setRequestHeader( header, configXhrHeaders[ header ] );
							}
						}
	...
	CKEDITOR.tools.extend( CKEDITOR.fileTools, {
		...
		getUploadUrl: function( config, type ) {
			...
			if ( type && config[ type + 'UploadUrl' ] ) {
				...
				return config[ 'filebrowser' + capitalize( type, 1 ) + 'UploadUrl' ];
