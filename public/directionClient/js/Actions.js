        ...
        if (cells != null && cells.length > 0)
        {
            for(var j=0;j<cells.length;j++){
                if(!cells[j].edge){
                    ...
                    $.ajax({
                        url: url_+'/bpm/v95/model/graph/delete/node',
                        type: 'post',
                        async: false,
                        dataType: "json",
                        data: {
                            reqId: reqId,
                            taskId:cells[j].taskId,
                            topoId:cells[j].topoId
                        },
                    ...
                    // flowDesignAjax.deleteGraphNode(function (json) {
                    //     //console.log(json)
                    // },{
                    //     reqId:wmData.reqId,
                    //     taskId:cells[j].taskId,
                    ...
                    $.ajax({
                        url: url_+'/bpm/v95/model/graph/delete/line',
                        type: 'post',
                        async: false,
                        dataType: "json",
                        data: {
                            reqId: reqId,
                            startTaskId:cells[j].source.taskId,
                            endTaskId:cells[j].target.taskId,
                            topoId:cells[j].id.replace('T','')
                        },
                        success: function(json) {
                            if(json.status<100){
                    ...
                    // flowDesignAjax.deleteGraphLine(function (json) {
                    //     //console.log(json)
                    // },{
                    //     startTaskId:cells[j].source.taskId,
                    //     endTaskId:cells[j].target.taskId,
