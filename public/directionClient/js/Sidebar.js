...
Sidebar.prototype.init = function() {
    ...
    var nodeList = [],
        zlist = [];
    nodeList = [ 'start', 'work', 'condition', 'abfinish', 'end','cc','subprocess', 'afbot' ,'alphaManual'];
    zlist = [ '', '', '', '', '', '', '','',''];
    if(window.isCustomer){
        nodeList = ['start', 'work', 'condition', 'end'];
    }
 ...
 */
Sidebar.prototype.gearImage = STENCIL_PATH + '/clipart/adduser.svg';
    ...
    if (this.enableTooltips && this.showTooltips) {
        if (this.currentElt != elt) {
            ...
            var show = mxUtils.bind(this, function() {
                ...
                var tooltip_cells = [ cells[0].clone() ];
                //
                tooltip_cells[0].setValue(getVertexDescByStyle(tooltip_cells[0].style));
                //this.graph2.addCells(cells);
                this.graph2.addCells(tooltip_cells);
                //this.graph2.addCells(cells);
                mxClient.NO_FO = fo;
...
Sidebar.prototype.createThumb = function(
    ...
    var s =
        Math.floor(
            Math.min((width - 2 * this.thumbBorder) / bounds.width, (height - 2 * this.thumbBorder) / bounds.height) *
            100
        ) / 100;
...
Sidebar.prototype.createDropHandler = function(cells, allowSplit, allowCellsInserted, bounds) {
    ...
    return mxUtils.bind(this, function(graph, evt, target, x, y) {
        if (graph.isEnabled()) {
            ...
            const _style = cells[0].style;
            let typeId = 0;
            var flag = true;
            if (_style.indexOf('start') !== -1) {
                typeId = 1;
            } else if (_style.indexOf('work') !== -1) {
                typeId = 6;
            } else if (_style.indexOf('condition') !== -1) {
                typeId = 4;
            } else if (_style.indexOf('abfinish') !== -1) {
                typeId = 13;
            } else if (_style.indexOf('end') !== -1) {
                typeId = 10;
            } else if (_style.indexOf('cc') !== -1) {
                typeId = 5;
            } else if (_style.indexOf('afbot') !== -1) {
                typeId = 8;
            } else if (_style.indexOf('alphaManual') !== -1) {
                typeId = 9;
            } else if (_style.indexOf('metadata') !== -1) {
                typeId = 15;
            } else if (_style.indexOf('subprocess') !== -1) {
                typeId = 7;
            }
            $.ajax({
                url: url_+'/bpm/v95/model/graph/create/node',
                type: 'post',
                async: false,
                dataType: "json",
                data: {
                    ...
                    style: cells[0].style,
                    type: typeId,
                    width: cells[0].geometry.width,
                    height: cells[0].geometry.height,
                    x: x,
                    y: y
                },
                success: function(json) {
                    var data = json.remap;
                    if (json.status < 100) {
                        cells[0].topoId = data.topoId;
                        cells[0].taskId = data.newTaskId;
                        if (typeId == 15) {
                            // setTimeout(function() {
                            //     graphClickFactory(_style, addCellTarget);
                            // }, 0);
                        }
                    }else{
                        flag = false;
            ...
            //console.log(3434)
            // 	flowDesignAjax.addGraphNode(function (json) {
            // 	var data = json.data
            // 	if(json.status==200){
            // 		if(data.status==200){
            // 			//console.log(data.data)
            // 			cells[0].topoId=data.data.topoId
            // 			cells[0].taskId = data.data.newTaskId
            // 		}
            // 	}
            // },{
            // 	reqId:1,
            // 	taskName:cells[0].value,
            // 	typeId
            // })
            try{
                if (cells.length > 0) {
    ...
    if (geo != null) {
        ...
        try {
            ...
            if (graph.model.isEdge(source)) {
                ...
                tmp = targets.slice();
                targets.push(
                    graph.insertEdge(null, null, '', source, targets[dropCellIndex], graph.createCurrentEdgeStyle())
                );
            }
