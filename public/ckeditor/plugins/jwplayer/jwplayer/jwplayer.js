...
(function(b) {
					...
					if (e.file) {
						...
						if (e.levels && e.levels.length) {
							for ( var d = 0; d < e.levels.length; d++) {
								if (e.levels[d].file
										&& b.utils.flashCanPlay(
												e.levels[d].file, e.provider)) {
									return true
								}
							}
						}
	...
	b.utils.flashCanPlay = function(c, e) {
		...
		if (b.utils.extensionmap[f] !== undefined
				&& b.utils.extensionmap[f].flash === undefined) {
			return false
		}
		return true
	};
		...
		if (!!c.canPlayType) {
			if (d) {
				...
				if (typeof f.file == "undefined"
						&& typeof f.levels == "undefined") {
					return true
				} else {
					if (f.file) {
						return b.utils.html5CanPlay(c, f.file, f.provider,
								f.playlistfile)
					} else {
						if (f.levels && f.levels.length) {
							for ( var e = 0; e < f.levels.length; e++) {
								if (f.levels[e].file
										&& b.utils.html5CanPlay(c,
												f.levels[e].file, f.provider,
												f.playlistfile)) {
									return true
								}
							}
	...
	b.utils.browserCanPlay = function(d, e) {
		...
		if (!e) {
			...
			if (b.utils.extensionmap[e] !== undefined
					&& b.utils.extensionmap[e].html5 === undefined) {
				return false
			} else {
				if (b.utils.extensionmap[e] !== undefined
						&& b.utils.extensionmap[e].html5 !== undefined) {
	...
	b.utils.downloadSupportsConfig = function(c) {
		if (c) {
			...
			if (typeof e.file == "undefined" && typeof e.levels == "undefined") {
				...
				if (e.file) {
					return b.utils.canDownload(e.file, e.provider,
							e.playlistfile)
				} else {
					if (e.levels && e.levels.length) {
						for ( var d = 0; d < e.levels.length; d++) {
							if (e.levels[d].file
									&& b.utils.canDownload(e.levels[d].file,
											e.provider, e.playlistfile)) {
								return true
							}
						}
					}
	...
	b.utils.hasFlash = function() {
		return (typeof navigator.plugins != "undefined" && typeof navigator.plugins["Shockwave Flash"] != "undefined")
	...
	b.utils.stretch = function(h, m, l, f, k, g) {
		if (typeof l == "undefined" || typeof f == "undefined"
				|| typeof k == "undefined" || typeof g == "undefined") {
			return
		}
		var d = l / k;
		var e = f / g;
		...
		case b.utils.stretching.EXACTFIT:
			b.utils.transform(m, [ "scale(", d, ",", e, ")",
					" translate(0px,0px)" ].join(""));
			c.width = k;
			c.height = g;
			break;
		default:
		...
		if (c == "#") {
			...
			if (c == ".") {
				if (d.getElementsByClassName) {
					...
					return a.utils.selectors.getElementsByTagAndClass("*", b
							.substr(1))
				}
			} else {
				if (b.indexOf(".") > 0) {
...
(function(b) {
	...
	b.api.PlayerAPI = function(d) {
		...
		this.addInternalListener = function(p, q) {
			p.jwAddEventListener(q, 'function(dat) { jwplayer("' + this.id
					+ '").dispatchEvent("' + q + '", dat); }')
		};
	...
	b.api.PlayerAPI.prototype = {
		...
		play : function(c) {
			if (typeof c == "undefined") {
				c = this.getState();
				if (c == b.api.events.state.PLAYING
						|| c == b.api.events.state.BUFFERING) {
		...
		onBufferFull : function(c) {
			return this.eventListener(b.api.events.JWPLAYER_MEDIA_BUFFER_FULL,
					c)
		},
		onError : function(c) {
...
(function(a) {
	...
	a.embed.Embedder.prototype = {
		...
		constructor : function(e) {
			...
			this.config = this.parseConfig(a.utils.extend({}, a.embed.defaults,
					d, this.api.config))
		},
			...
			if (e && e.type) {
				...
				case "flash":
					if (a.utils.flashSupportsConfig(this.config)) {
						if (this.config.file && !this.config.provider) {
							switch (a.utils.extension(this.config.file)
									.toLowerCase()) {
							case "webm":
							case "ogv":
							case "ogg":
						...
						var j = a.embed.embedFlash(document
								.getElementById(this.api.id), e, this.config);
				...
				case "html5":
					if (a.utils.html5SupportsConfig(this.config)) {
						var g = a.embed.embedHTML5(document
								.getElementById(this.api.id), e, this.config);
						this.api.container = document
								.getElementById(this.api.id);
				...
				case "download":
					if (a.utils.downloadSupportsConfig(this.config)) {
						var f = a.utils
								.getFirstPlaylistItemFromConfig(this.config);
						var d = a.embed.embedDownloadLink(document
								.getElementById(this.api.id), e, this.config);
						this.api.container = document
								.getElementById(this.api.id);
		...
		parseConfig : function(d) {
			...
			if (h.components) {
				if (typeof h.plugins == "object") {
					h = a.utils
							.extend(h, a.embed.parseComponents(h.components))
				}
			}
			return h
		}
	...
	a.embed.embedFlash = function(f, h, k) {
		...
		if (a.utils.isIE()) {
			var j = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'
					+ 575
					+ '" height="'
					+ 460
					+ '" id="'
	...
	a.embed.jsonToFlashvars = function(f) {
		var d = f.netstreambasepath ? "" : "netstreambasepath="
...
(function(b) {
	...
	b.html5.view = function(q, o, e) {
		...
		function y() {
			...
			function C() {
				if (t.skin.getComponentSettings("display")
						&& t.skin.getComponentSettings("display").backgroundcolor) {
		...
		function j() {
			for ( var C = 0; C < w.plugins.order.length; C++) {
				...
				if (w.plugins.object[D].getDisplayElement !== undefined) {
					w.plugins.object[D].height = h(w.plugins.object[D]
							.getDisplayElement().style.height);
					w.plugins.object[D].width = h(w.plugins.object[D]
		...
		function p() {
			n = setInterval(
					function() {
						if ((typeof w.width == "string" && w.width
								.lastIndexOf("%") > -1)
								|| (typeof w.height == "string" && w.height
										.lastIndexOf("%") > -1)) {
							return
						}
						if (v.width
								&& v.height
								&& (w.width !== h(v.width) || w.height !== h(v.height))) {
		...
		function g(C) {
			...
			case 27:
				...
				break;
			case 32:
				if (t.jwGetState() != b.api.events.state.IDLE
						&& t.jwGetState() != b.api.events.state.PAUSED) {
					t.jwPause()
				} else {
					t.jwPlay()
				}
		...
		function k(E, F) {
			...
			if (typeof w.width == "string" && w.width.lastIndexOf("%") > -1) {
				percentage = parseFloat(w.width.substring(0, w.width
						.lastIndexOf("%"))) / 100;
				C = Math.round(window.innerWidth * percentage)
			}
			if (typeof w.height == "string" && w.height.lastIndexOf("%") > -1) {
				percentage = parseFloat(w.height.substring(0, w.height
						.lastIndexOf("%"))) / 100;
		...
		function x() {
			...
			if (w.getMedia().getDisplayElement().videoWidth == 0
					|| w.getMedia().getDisplayElement().videoHeight == 0) {
				return
			}
			var C, E;
			if (r.style.width.toString().lastIndexOf("%") > -1
					|| r.style.width.toString().lastIndexOf("%") > -1) {
			...
			b.utils.stretch(t.jwGetStretching(), w.getMedia()
					.getDisplayElement(), C, E, w.getMedia()
					.getDisplayElement().videoWidth, w.getMedia()
					.getDisplayElement().videoHeight)
		}
		function d(D) {
			var E = {
				position : "absolute",
	...
	function a(d) {
		return ([ b.html5.view.positions.TOP, b.html5.view.positions.RIGHT,
				b.html5.view.positions.BOTTOM, b.html5.view.positions.LEFT ]
				.toString().indexOf(d.toUpperCase()) > -1)
	}
...
(function(a) {
	...
	a.html5.controlbar = function(j, L) {
		var i = j;
		var A = a.utils.extend({}, b,
				i.skin.getComponentSettings("controlbar"), L);
		...
		function H() {
			...
			var W = {
				...
				height : i.skin.getSkinElement("controlbar", "background").height,
				background : " url("
						+ i.skin.getSkinElement("controlbar", "background").src
		...
		function o() {
			var W = [ "timeSlider", "volumeSlider", "timeSliderRail",
					"volumeSliderRail" ];
			for ( var X in W) {
				var V = W[X];
		...
		function g() {
			if (i.jwGetState() == a.api.events.state.IDLE
					|| i.jwGetState() == a.api.events.state.PAUSED) {
				if (A.idlehide) {
					return false
				}
				return true
		...
		function z(Z, ab) {
			...
			case "elapsed":
				...
				break;
			case "time":
				W = i.skin.getSkinElement("controlbar", "timeSliderCapLeft") === undefined ? 0
						: i.skin.getSkinElement("controlbar",
								"timeSliderCapLeft").width;
				X = i.skin.getSkinElement("controlbar", "timeSliderCapRight") === undefined ? 0
						: i.skin.getSkinElement("controlbar",
								"timeSliderCapRight").width;
				Y = ab == "left" ? W : X;
				V = i.skin.getSkinElement("controlbar", "timeSliderRail").width
						+ W + X;
				ad = {
					height : i.skin.getSkinElement("controlbar", "background").height,
					position : "absolute",
			...
			case "fullscreen":
				...
				break;
			case "volume":
				W = i.skin.getSkinElement("controlbar", "volumeSliderCapLeft") === undefined ? 0
						: i.skin.getSkinElement("controlbar",
								"volumeSliderCapLeft").width;
				X = i.skin.getSkinElement("controlbar", "volumeSliderCapRight") === undefined ? 0
						: i.skin.getSkinElement("controlbar",
								"volumeSliderCapRight").width;
				Y = ab == "left" ? W : X;
				V = i.skin.getSkinElement("controlbar", "volumeSliderRail").width
						+ W + X;
				ad = {
					height : i.skin.getSkinElement("controlbar", "background").height,
					position : "absolute",
		...
		function v(Y, ac, ab, Z, V) {
			if ((i.skin.getSkinElement("controlbar", Y) !== undefined
					|| Y.indexOf("Text") > 0 || Y.indexOf("divider") === 0)
					&& !(Y.indexOf("divider") === 0 && s.indexOf("divider") === 0)) {
				s = Y;
				var X = {
					height : i.skin.getSkinElement("controlbar", "background").height,
					position : "absolute",
				...
				if ((Y.indexOf("next") === 0 || Y.indexOf("prev") === 0)
				...
				if (Y.indexOf("Text") > 0) {
					...
					X.font = A.fontsize
							+ "px/"
							+ (i.skin
									.getSkinElement("controlbar", "background").height + 1)
							+ "px " + A.font;
					X.color = A.fontcolor;
					X.textAlign = "center";
					...
					if (Y.indexOf("divider") === 0) {
						X.background = "url("
								+ i.skin
										.getSkinElement("controlbar", "divider").src
								+ ") repeat-x center left";
						aa = i.skin.getSkinElement("controlbar", "divider").width
					} else {
						X.background = "url("
								+ i.skin.getSkinElement("controlbar", Y).src
								+ ") repeat-x center left";
					...
					if (i.skin.getSkinElement("controlbar", Y + "Over") !== undefined) {
						W.onmouseover = function(ad) {
							W.style.backgroundImage = [
									"url(",
									i.skin.getSkinElement("controlbar", Y
											+ "Over").src, ")" ].join("")
						};
		...
		function r(W) {
			...
			var V = isNaN(Math.round(X * e / 100)) ? 0 : Math
					.round(X * e / 100);
		...
		function p(V) {
			if (V.newstate == a.api.events.state.BUFFERING
					|| V.newstate == a.api.events.state.PLAYING) {
				_show(J.pauseButton);
				_hide(J.playButton)
			} else {
				_hide(J.pauseButton);
		...
		function D(Y) {
			...
			var W = (h === Q === 0) ? 0 : h / Q;
			var V = isNaN(Math.round(c.timeSliderRail.width * W)) ? 0 : Math
					.round(c.timeSliderRail.width * W);
			var X = V;
		...
		function m(V) {
			str = "00:00";
			if (V > 0) {
				str = Math.floor(V / 60) < 10 ? "0" + Math.floor(V / 60) + ":"
						: Math.floor(V / 60) + ":";
				str += Math.floor(V % 60) < 10 ? "0" + Math.floor(V % 60)
						: Math.floor(V % 60)
			}
		...
		function l() {
			...
			for ( var V in document.getElementById(P.id + "_elements").childNodes) {
				...
				if (X[V].id.indexOf(P.id + "_divider") === 0
						&& W.id.indexOf(P.id + "_divider") === 0) {
		...
		function u() {
			...
			if (A.position.toUpperCase() == a.html5.view.positions.OVER
				...
				W.width -= 2 * A.margin;
				W.top = w
						- i.skin.getSkinElement("controlbar", "background").height
						- A.margin;
			...
			V.width = W.width
					- i.skin.getSkinElement("controlbar", "capLeft").width
					- i.skin.getSkinElement("controlbar", "capRight").width;
			var X = i.skin.getSkinElement("controlbar", "timeSliderCapLeft") === undefined ? 0
					: i.skin.getSkinElement("controlbar", "timeSliderCapLeft").width;
		...
		function k(Z) {
			if (J.volumeSliderRail !== undefined) {
				...
				var Y = parseInt(J.volumeSliderRail.style.width.replace("px",
						""), 10);
				var V = isNaN(Math.round(Y * X)) ? 0 : Math.round(Y * X);
				var aa = parseInt(J.volumeSliderRail.style.right.replace("px",
						""), 10);
				var W = i.skin.getSkinElement("controlbar",
						"volumeSliderCapLeft") === undefined ? 0
						: i.skin.getSkinElement("controlbar",
								"volumeSliderCapLeft").width;
...
(function(b) {
	...
	b.html5.controller = function(t, r, e, q) {
		...
		var A = true;
		var c = -1;
		var u = (y.config.debug !== undefined)
				&& (y.config.debug.toString().toLowerCase() == "console");
			...
			try {
				if (y.playlist[y.item].levels[0].file.length > 0) {
					if (A || y.state == b.api.events.state.IDLE) {
						y.addEventListener(
								b.api.events.JWPLAYER_MEDIA_BUFFER_FULL,
								function() {
									y.getMedia().play()
								});
						y.addEventListener(b.api.events.JWPLAYER_MEDIA_TIME,
								function(E) {
									if (E.position >= y.playlist[y.item].start
											&& c >= 0) {
						...
						if (y.config.repeat) {
							y.addEventListener(
									b.api.events.JWPLAYER_MEDIA_COMPLETE,
									function(E) {
										setTimeout(n, 25)
									})
						}
		...
		function j() {
			try {
				if (y.playlist[y.item].levels[0].file.length > 0
						&& y.state != b.api.events.state.IDLE) {
					y.getMedia().stop()
				}
				return true
			} catch (D) {
		...
		function g() {
			try {
				...
				if (y.state != b.api.events.state.PLAYING
						&& y.state != b.api.events.state.BUFFERING) {
					p()
				}
				return true
			} catch (D) {
			...
			try {
				if (y.playlist[E].levels[0].file.length > 0) {
					...
					if (F == b.api.events.state.PLAYING
							|| F == b.api.events.state.BUFFERING
							|| y.config.chromeless) {
						p()
					}
				}
...
(function(a) {
	...
	a.html5.display = function(o, y) {
		...
		var p;
		var j;
		var n = v.skin.getComponentSettings("display").bufferrotation === undefined ? 15
				: parseInt(
						v.skin.getComponentSettings("display").bufferrotation,
						10);
		var f = v.skin.getComponentSettings("display").bufferinterval === undefined ? 100
				: parseInt(
						v.skin.getComponentSettings("display").bufferinterval,
						10);
		var d = {
			display : {
				style : {
			...
			display_icon : {
				style : {
					cursor : "pointer",
					position : "absolute",
					top : ((v.skin.getSkinElement("display", "background").height - v.skin
							.getSkinElement("display", "playIcon").height) / 2),
					left : ((v.skin.getSkinElement("display", "background").width - v.skin
			...
			display_iconBackground : {
				style : {
					...
					border : 0,
					backgroundImage : ([ "url(",
							v.skin.getSkinElement("display", "background").src,
							")" ]).join(""),
		...
		this.resize = function(C, B) {
			...
			_css(e.display_iconBackground,
					{
						top : ((z - v.skin.getSkinElement("display",
								"background").height) / 2),
						left : ((g - v.skin.getSkinElement("display",
								"background").width) / 2)
					});
			i();
			l({})
		...
		function h(B) {
			...
			e.display_icon.style.backgroundImage = ([ "url(",
					v.skin.getSkinElement("display", B).src, ")" ]).join("");
			_css(
					e.display_icon,
					{
						display : "block",
						...
						top : (v.skin.getSkinElement("display", "background").height - v.skin
								.getSkinElement("display", B).height) / 2,
						left : (v.skin.getSkinElement("display", "background").width - v.skin
								.getSkinElement("display", B).width) / 2
					});
			if (v.skin.getSkinElement("display", B + "Over") !== undefined) {
				e.display_icon.onmouseover = function(C) {
					e.display_icon.style.backgroundImage = [ "url(",
							v.skin.getSkinElement("display", B + "Over").src,
							")" ].join("")
				};
				e.display_icon.onmouseout = function(C) {
					e.display_icon.style.backgroundImage = [ "url(",
							v.skin.getSkinElement("display", B).src, ")" ]
							.join("")
				}
			} else {
		...
		function s(B) {
			...
			e.display_text.style.top = ((z - e.display_text
		...
		function l(B) {
			if ((B.type == a.api.events.JWPLAYER_PLAYER_STATE || B.type == a.api.events.JWPLAYER_PLAYLIST_ITEM)
					&& j) {
				j = false;
			...
			case a.api.events.state.IDLE:
				if (v.jwGetPlaylist()[v.jwGetItem()].image) {
					...
					e.display_image.src = a.utils.getAbsolutePath(v
							.jwGetPlaylist()[v.jwGetItem()].image)
				} else {
					u()
				}
				h("playIcon");
			...
			try {
				for ( var listenerIndex = 0; listenerIndex < _listeners[type].length; listenerIndex++) {
					if (_listeners[type][lisenterIndex].toString() == listener
							.toString()) {
						_listeners[type]
								.slice(lisenterIndex, lisenterIndex + 1);
						break
					}
				}
			} catch (err) {
		...
		this.removeGlobalListener = function(listener) {
			try {
				for ( var globalListenerIndex = 0; globalListenerIndex < _globallisteners.length; globalListenerIndex++) {
					if (_globallisteners[globalListenerIndex].toString() == listener
							.toString()) {
						_globallisteners.slice(globalListenerIndex,
								globalListenerIndex + 1);
						break
					}
				}
			} catch (err) {
				...
				for ( var listenerIndex = 0; listenerIndex < _listeners[type].length; listenerIndex++) {
					try {
						...
						jwplayer.utils.log(
								...
								_listeners[type][listenerIndex].listener, err)
					}
...
(function(a) {
	...
	a.html5.mediavideo = function(f, D) {
		...
		function l(Q) {
			...
			if ((Q.target.error && Q.target.tagName.toLowerCase() == "video")
					|| Q.target.parentNode.error
					&& Q.target.parentNode.tagName.toLowerCase() == "video") {
				var O = Q.target.error === undefined ? Q.target.parentNode.error
						: Q.target.error;
				switch (O.code) {
		...
		function J(V) {
			...
			for ( var P = 0; P < V.levels.length; P++) {
				...
				if (O.type === undefined) {
					...
					if (a.utils.extensionmap[U] !== undefined
							&& a.utils.extensionmap[U].html5 !== undefined) {
			...
			if (L === 0) {
				...
				I
						.sendEvent(
								a.api.events.JWPLAYER_ERROR,
								{
									error : "The video could not be loaded because the format is not supported by your browser: "
											+ j()
								})
			}
			if (h.config.chromeless) {
	...
	jwplayer.html5.model = function(api, container, options) {
		...
		for ( var option in options) {
			if (typeof options[option] == "string") {
				...
				options[option] = jwplayer.utils.typechecker(options[option],
						type)
			}
		...
		for ( var pluginIndex in _model.plugins.order) {
			...
			_model.plugins.config[pluginName] = _model.plugins.config[pluginName] === undefined ? pluginConfig
			...
			for ( var plugin in _model.plugins.order) {
				try {
					if (jwplayer.html5[_model.plugins.order[plugin]] !== undefined) {
						_model.plugins.object[_model.plugins.order[plugin]] = new jwplayer.html5[_model.plugins.order[plugin]](
								_api,
								_model.plugins.config[_model.plugins.order[plugin]])
					} else {
						if (window[_model.plugins.order[plugin]] !== undefined) {
							_model.plugins.object[_model.plugins.order[plugin]] = new window[_model.plugins.order[plugin]](
									_api,
									_model.plugins.config[_model.plugins.order[plugin]])
						} else {
					...
					jwplayer.utils.log("Could not setup "
							+ _model.plugins.order[plugin])
				}
			}
		};
		return _model
	...
	a.html5.skinloader = function(f, n, i) {
		...
		function d(w) {
			...
			for ( var F = 0; F < C.length; F++) {
				...
				m[A] = z;
				var E = C[F].getElementsByTagName("elements")[0]
...
(function(a) {
	a.html5.api = function(b, k) {
		...
		function d() {
			if (i.state == a.api.events.state.PLAYING
					|| i.state == a.api.events.state.BUFFERING) {
				h.pause()
			} else {
				h.play()
			}
